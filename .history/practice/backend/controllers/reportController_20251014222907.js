...
/* ------------------------------------------------
   ✅ Resolve Report - FIXED: Handle invalid user IDs
------------------------------------------------ */
exports.resolveReport = async (req, res) => {
  try {
    // FIXED: Add proper validation for req.body
    if (!req.body) {
    ...
    // FIXED: Handle user lookup safely - don't try to find user if resolvedBy is not a valid ObjectId
    let resolverName = "Admin";
    let resolverIdNumber = "N/A";

    // Only try to find user if resolvedBy is a valid MongoDB ObjectId (24 character hex string)
    if (resolvedBy && /^[0-9a-fA-F]{24}$/.test(resolvedBy)) {
      try {
        ...
        console.warn("Could not find user for resolvedBy:", resolvedBy);
        // Continue with default admin values
      }
    }
...
/* ------------------------------------------------
   ✅ Admin Updates Report Status / Action Taken - FIXED
------------------------------------------------ */
exports.updateReportStatus = async (req, res) => {
  try {
    ...
    // FIXED: Handle user lookup safely
    ...
    if (updatedBy && /^[0-9a-fA-F]{24}$/.test(updatedBy)) {
      try {
        ...
        console.warn("Could not find user for updatedBy:", updatedBy);
      }
    }
...
/* ------------------------------------------------
   ✅ Archive Report - FIXED: Handle invalid user IDs
------------------------------------------------ */
exports.archiveReport = async (req, res) => {
  try {
    ...
    if (archivedBy && /^[0-9a-fA-F]{24}$/.test(archivedBy)) {
      try {
        ...
        console.warn("Could not find user for archivedBy:", archivedBy);
      }
    }
...
/* ------------------------------------------------
   ✅ Restore Report - FIXED: Handle invalid user IDs
------------------------------------------------ */
exports.restoreReport = async (req, res) => {
  try {
    ...
    if (restoredBy && /^[0-9a-fA-F]{24}$/.test(restoredBy)) {
      try {
        ...
        console.warn("Could not find user for restoredBy:", restoredBy);
      }
    }
...
/* ------------------------------------------------
   ✅ Delete Archived Report - FIXED: Handle invalid user IDs
------------------------------------------------ */
exports.deleteArchivedReport = async (req, res) => {
  try {
    ...
    if (deletedBy && /^[0-9a-fA-F]{24}$/.test(deletedBy)) {
      try {
        ...
        console.warn("Could not find user for deletedBy:", deletedBy);
      }
    }
...
/* ------------------------------------------------
   ✅ Assign Report to Staff - FIXED: Handle invalid user IDs
------------------------------------------------ */
exports.assignReport = async (req, res) => {
  try {
    ...
    if (assignedBy && /^[0-9a-fA-F]{24}$/.test(assignedBy)) {
      try {
        ...
        console.warn("Could not find user for assignedBy:", assignedBy);
      }
    }
