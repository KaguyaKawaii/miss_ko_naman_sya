...
exports.createReport = async (req, res) => {
  try {
    ...
    if (assignedTo && assignedStaff) {
      ...
      // ✅ Log auto-assignment with staff name
      await logAction(
        assignedTo, // staff ID
        assignedStaff.id_number, // staff ID number
        assignedStaff.name, // staff name
        "Staff Auto-Assigned",
        `Report ${newReport._id} for ${floor} - ${room} auto-assigned to ${assignedStaff.name}`
      );
    }
    ...
    res.status(201).json({
      message: assignedTo
        ? `Report submitted and auto-assigned to ${assignedStaff?.name}`
