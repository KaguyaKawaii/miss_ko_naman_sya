...
exports.createReport = async (req, res) => {
  try {
    ...
    // 3️⃣ ✅ FIX: Find all admin users and create notifications for them only
    const adminUsers = await User.find({ role: "admin" }); // Adjust role field as needed
    ...
    for (const admin of adminUsers) {
      const notification = await Notification.create({
        userId: admin._id, // ✅ Specific admin user
      ...
      // Populate and emit to specific admin (if socket.io supports user-specific emits)
      ...
      // ✅ Emit to specific admin user only
      if (io) {
        io.to(admin._id.toString()).emit("notification", populatedNotification);
      }
    }
    ...
    res.status(201).json({
      ...
      report: newReport,
      notifications, // Return all created notifications
    });
  } catch (err) {
