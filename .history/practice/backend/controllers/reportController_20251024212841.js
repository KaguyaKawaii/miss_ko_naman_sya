...
exports.createReport = async (req, res) => {
  try {
    ...
    // 3️⃣ ✅ FIX: Get all admins from the admins collection
    ...
    for (const admin of admins) {
      const notification = await Notification.create({
        // Don't set userId since admins are in different collection
        ...
        isRead: false,
        dismissed: false,
        isForAdmins: true, // ✅ Flag for admin notifications
        targetAdmin: admin._id, // ✅ Reference to admin
      });
      ...
      if (io) {
        io.to(`admin-${admin._id}`).emit("notification", populatedNotification);
      }
    }
...
exports.getAdminNotifications = async (req, res) => {
  try {
    const { adminId } = req.params; // Get admin ID from params
    
    const notifications = await Notification.find({ 
      $or: [
        { isForAdmins: true }, // General admin notifications
        { targetAdmin: adminId } // Specific admin notifications
      ],
      dismissed: false 
    })
    .populate("reportId", "category floor room status")
    .sort({ createdAt: -1 });
