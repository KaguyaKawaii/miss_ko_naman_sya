...
/* ------------------------------------------------
   ✅ FLOOR ACCESS VALIDATION UTILITIES
------------------------------------------------ */
const validateFloorAccess = (user, floor) => {
  ...
  if (normalizedFloor.includes('ground')) {
    return userProgram.includes('master') || 
  ...
  if (normalizedFloor.includes('2nd') || normalizedFloor.includes('second')) {
    return userDepartment.includes('law') || 
...
// ✅ ADD THIS - Export the floor access validation function
exports.validateFloorAccess = async (req, res) => {
  try {
    const { location, participantIds } = req.body;

    if (!location || !participantIds || !Array.isArray(participantIds)) {
      return res.status(400).json({ 
        valid: false, 
        message: "Location and participant IDs are required." 
      });
    }
...
// controllers/reservationController.js - Fixed version
