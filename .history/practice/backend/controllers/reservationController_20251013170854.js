...
exports.validateFloorAccess = async (req, res) => {
  try {
    ...
    // ✅ FIXED: Process each participant with proper ID number lookup
    for (const participantId of validParticipantIds) {
      try {
        console.log('🔍 Looking up participant by ID number:', participantId);
        
        // ✅ FIXED: Only search by id_number, not _id
        const participantUser = await User.findOne({ 
          id_number: participantId.toString().trim()
        }).select('name id_number department course program yearLevel verified');

        if (!participantUser) {
          console.log('❌ Participant not found with ID number:', participantId);
