...
for (const participant of enrichedParticipants) {
  ...
  if (participantUser) {
    await notificationService.createNotification(
      {
        userId: participantUser._id, // Send to participant's user ID
        reservationId: reservation._id,
        type: "reservation",
        ...
        startTime: time,
        endTime: new Date(endDatetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        message: `You have been added as a participant to a reservation for ${roomName} on ${date} at ${time}`
      },
      req.app.get("io")
    );
  }
      ...
      for (const participant of reservation.participants) {
        ...
        if (participantUser?.email) {
          try {
            ...
            console.log('✅ Expiration email sent to participant:', participantUser.name);
          } catch (emailError) {
