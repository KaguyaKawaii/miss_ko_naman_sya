...
/* ------------------------------------------------
   ✅ CHECK AND MARK EXPIRED RESERVATIONS + NOTIFY (FIXED)
------------------------------------------------ */
exports.checkExpiredReservations = async (req, res) => {
  try {
    ...
    // FIXED: Find reservations that should be expired based on end time OR no check-in
    const expiringReservations = await Reservation.find({
      status: { $in: ["Pending", "Approved", "Ongoing"] },
      $or: [
        // Reservations that have ended (endDatetime is in the past)
        { endDatetime: { $lte: now } },
    ...
    // Log details of each reservation being expired
    expiringReservations.forEach(reservation => {
      const reason = reservation.endDatetime <= now 
        ? `End time passed (${reservation.endDatetime})` 
        : `No check-in within 15 minutes of start time (${reservation.datetime})`;
    ...
    for (const reservation of expiringReservations) {
      ...
      // ✅ FIXED: Send expiration emails to participants
      for (const participant of reservation.participants) {
        if (participant.id_number === reservation.userId.id_number) continue;
