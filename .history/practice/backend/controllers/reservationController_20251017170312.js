...
/* ------------------------------------------------
   ✅ CHECK AND MARK EXPIRED RESERVATIONS + NOTIFY (FIXED FOR PENDING RESERVATIONS)
------------------------------------------------ */
exports.checkExpiredReservations = async (req, res) => {
  try {
    ...
    const expiringReservations = await Reservation.find({
      status: { $in: ["Pending", "Approved", "Ongoing"] },
      $or: [
        // ✅ FIXED: ANY reservation that has ended (endDatetime is in the past) - INCLUDES PENDING!
        { endDatetime: { $lte: now } },
    ...
    for (const reservation of expiringReservations) {
      const reason = reservation.endDatetime <= now
        ? `has ended. (Reservation was still ${reservation.status} when it expired)`
