...
// Add these exports to your reservationController.js

// ✅ GET ALL USERS (for dashboard)
exports.getAllUsers = async (req, res) => {
  try {
    const users = await User.find({})
      .select('name email id_number role department course program verified')
      .sort({ createdAt: -1 });
...
exports.getAllRooms = async (req, res) => {
  try {
    const rooms = await Room.find({ isActive: true })
      .select('room floor capacity isActive')
      .sort({ floor: 1, room: 1 });
