...
/* ------------------------------------------------
   ✅ REMOVE PARTICIPANT (WITHOUT REPLACEMENT)
------------------------------------------------ */
exports.removeParticipant = async (req, res) => {
  try {
    ...
    if (!reservationId || !participantId) {
      return res.status(400).json({ message: "Reservation ID and participant ID are required." });
    }
    ...
    if (reservation.status !== "Pending" && reservation.status !== "Approved") {
      return res.status(400).json({ 
        message: "Cannot remove participants from reservations that are not Pending or Approved." 
      });
    }
    ...
    await notificationService.createNotification(
      {
        ...
        targetRole: "user",
        roomName: reservation.roomName,
        extraNote: `Removed ${removedParticipant.name}`
      },
      req.app.get("io")
    );
    ...
    res.json({
      ...
      reservation,
      removedParticipant: removedParticipant.name
    });
