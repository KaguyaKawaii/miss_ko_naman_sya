...
/* ------------------------------------------------
   ✅ REPLACE CANCELLED PARTICIPANT
------------------------------------------------ */
exports.replaceParticipant = async (req, res) => {
  try {
    const { reservationId } = req.params;
    const { cancelledParticipantId, newParticipantId } = req.body;

    if (!reservationId || !cancelledParticipantId || !newParticipantId) {
      return res.status(400).json({ message: "Reservation ID, cancelled participant ID, and new participant ID are required." });
    }
    ...
    if (reservation.status !== "Pending" && reservation.status !== "Approved") {
      return res.status(400).json({ 
        message: "Cannot modify participants for reservations that are not Pending or Approved." 
      });
    }

    // Find the cancelled participant
    const cancelledParticipantIndex = reservation.participants.findIndex(
      p => p.id_number === cancelledParticipantId.toString()
    );

    if (cancelledParticipantIndex === -1) {
      return res.status(404).json({ message: "Cancelled participant not found in this reservation." });
    }

    // Find the new participant
    const newParticipantUser = await User.findOne({ 
      id_number: newParticipantId.toString().trim() 
    });

    if (!newParticipantUser) {
      return res.status(404).json({ message: "New participant not found." });
    }

    if (!newParticipantUser.verified) {
      return res.status(400).json({ message: "New participant account is not verified." });
    }

    // ✅ FLOOR ACCESS VALIDATION: Check new participant eligibility
    if (!validateFloorAccess(newParticipantUser, reservation.location)) {
      return res.status(400).json({
        message: `New participant ${newParticipantUser.name} does not have access to ${reservation.location}. ${getFloorRestrictionMessage(reservation.location)}`
      });
    }

    // Check if new participant has conflicting reservations
    const now = new Date();
    const conflictingReservations = await Reservation.find({
      $or: [
    ...
    if (conflictingReservations.length > 0) {
      return res.status(400).json({ 
        message: `New participant ${newParticipantUser.name} has a conflicting reservation.` 
      });
    }

    // Check if new participant is already in this reservation
    const isAlreadyParticipant = reservation.participants.some(
      p => p.id_number === newParticipantUser.id_number
    );

    if (isAlreadyParticipant) {
      return res.status(400).json({ 
        message: "New participant is already in this reservation." 
      });
    }

    // Check if new participant is the main user
    const mainUser = await User.findById(reservation.userId);
    if (newParticipantUser.id_number === mainUser.id_number) {
      return res.status(400).json({ 
        message: "New participant cannot be the main reserver." 
      });
    }

    // Store the cancelled participant details for logging
    const cancelledParticipant = reservation.participants[cancelledParticipantIndex];

    // Replace the participant
    reservation.participants[cancelledParticipantIndex] = {
    ...
    // ✅ Log participant replacement
    await logAction(
      reservation.userId,
      mainUser.id_number,
      mainUser.name,
      "Participant Replaced",
      `Replaced ${cancelledParticipant.name} with ${newParticipantUser.name} in reservation for ${reservation.roomName}`
    );
    ...
    await notificationService.createNotification(
      {
        ...
        targetRole: "user",
        roomName: reservation.roomName,
        extraNote: `Replaced ${cancelledParticipant.name} with ${newParticipantUser.name}`
      },
      req.app.get("io")
    );
    ...
    await notificationService.createNotification(
      {
        ...
        startTime: new Date(reservation.datetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        endTime: new Date(reservation.endDatetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        extraNote: `You were added by ${mainUser.name}`
      },
      req.app.get("io")
    );
    ...
    try {
      await sendEmail({
        ...
        subject: "You've been added to a reservation",
        html: generateReservationEmail({
      ...
      console.warn('⚠️ Failed to send email to new participant:', emailError.message);
    }

    res.json({
      success: true,
      message: "Participant replaced successfully",
      reservation,
      cancelledParticipant: cancelledParticipant.name,
      newParticipant: newParticipantUser.name
    });

  } catch (err) {
    console.error("Participant replacement error:", err);
    res.status(500).json({ message: "Failed to replace participant." });
  }
};
...
exports.removeParticipant = async (req, res) => {
  try {
    ...
    console.log('🔄 Removing participant request:', { reservationId, participantId });
    ...
    console.log('📋 Reservation found:', {
      ...
      room: reservation.roomName,
      status: reservation.status,
      currentParticipants: reservation.participants.length
    });
    ...
    if (reservation.status !== "Pending") {
      return res.status(400).json({ 
        success: false,
        message: "Can only remove participants from Pending reservations." 
      });
    }

    // Check minimum group size (4 including main user)
    const currentGroupSize = reservation.participants.length + 1;
    if (currentGroupSize <= 4) {
      return res.status(400).json({ 
        success: false,
        message: "Cannot remove participant. Minimum group size is 4 users including the main reserver." 
      });
    }

    // Find the participant to remove
    const participantIndex = reservation.participants.findIndex(
      p => p.id_number === participantId.toString()
    );
    ...
    console.log('👤 Participant to remove:', removedParticipant);
    ...
    await reservation.save();
    console.log('✅ Participant removed, new participant count:', reservation.participants.length);
    ...
    await notificationService.createNotification(
      {
        ...
        targetRole: "user",
        roomName: reservation.roomName,
        extraNote: `Removed ${removedParticipant.name} from reservation`
      },
      req.app.get("io")
    );

    // ✅ Find and notify the removed participant
    const removedUser = await User.findOne({ id_number: removedParticipant.id_number });
    if (removedUser) {
      console.log('🔔 Notifying removed participant:', removedUser.name);
      ...
      await notificationService.createNotification(
        {
          ...
          targetRole: "user",
          roomName: reservation.roomName,
          extraNote: `You were removed from the reservation by ${reservation.userId.name}`
        },
        req.app.get("io")
      );
      ...
      try {
        await sendEmail({
          ...
          html: generateReservationEmail({
            ...
            participants: reservation.participants,
            isParticipant: true,
            extraNote: `You were removed from this reservation by ${reservation.userId.name}`
          })
        });
        console.log('📧 Removal email sent to:', removedUser.email);
      } catch (emailError) {
        console.warn('⚠️ Failed to send removal email:', emailError.message);
      }
    }

    console.log('🎉 Participant removal completed successfully');
    res.json({
      success: true,
      ...
      removedParticipant: {
        ...
        id_number: removedParticipant.id_number
      }
    });
