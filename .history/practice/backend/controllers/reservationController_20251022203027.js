...
exports.removeParticipant = async (req, res) => {
  try {
    const { id: reservationId, participantId } = req.params;
    const { userId } = req.body; // Main reserver ID

    console.log('🔄 Removing participant from reservation:', {
      reservationId,
      participantId,
      userId
    });
    ...
    if (!userId) {
      return res.status(400).json({
        success: false,
        message: "Main reserver ID is required"
      });
    }
    ...
    console.log('📋 Reservation details:', {
      ...
      mainReserver: reservation.userId._id.toString(),
      currentParticipants: reservation.participants.length
    });

    // ✅ VALIDATION: Check if user is the main reserver
    if (reservation.userId._id.toString() !== userId) {
      return res.status(403).json({
        success: false,
        message: "Only the main reserver can remove participants"
      });
    }

    // ✅ VALIDATION: Check if reservation is in Pending status
    if (reservation.status !== "Pending") {
      return res.status(400).json({
        success: false,
        message: "Participants can only be removed from Pending reservations"
      });
    }

    // ✅ VALIDATION: Check minimum group size (4 users including main reserver)
    const currentGroupSize = reservation.participants.length + 1; // +1 for main reserver
    if (currentGroupSize <= 4) {
      return res.status(400).json({
        success: false,
        message: "Cannot remove participant - minimum group size is 4 users (including main reserver)"
      });
    }

    // Find the participant to remove
    const participantIndex = reservation.participants.findIndex(
      p => p.id_number === participantId || p._id?.toString() === participantId
    );
    ...
    console.log('🗑️ Removing participant:', removedParticipant);
    ...
    console.log('✅ Participant removed successfully. New group size:', reservation.numUsers);
    ...
    try {
      await logAction(
        ...
        reservation.userId.name,
        "Participant Removed",
        `Removed participant ${removedParticipant.name} (ID: ${removedParticipant.id_number}) from reservation for ${reservation.roomName}`
      );
      console.log('📝 Removal action logged');
    } catch (logError) {
      console.warn("⚠️ Failed to log removal action:", logError.message);
    }
      ...
      if (removedUser) {
        ...
        try {
          await sendEmail({
            ...
            subject: "Removed from Study Room Reservation",
            html: generateRemovalEmail({
              toName: removedUser.name,
              reservation: reservation,
              removedBy: reservation.userId.name,
              formattedDate: reservation.date,
              time: `${new Date(reservation.datetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - ${new Date(reservation.endDatetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`,
              roomName: reservation.roomName,
        ...
        console.warn('⚠️ Removed participant user not found:', removedParticipant.id_number);
      }
    } catch (notifError) {
      console.warn("⚠️ Failed to create removal notification:", notifError.message);
    }
    ...
    res.json({
      ...
      newGroupSize: reservation.numUsers
    });
