...
exports.removeParticipant = async (req, res) => {
  try {
    ...
    console.log('🔄 Removing participant from reservation:', { id, participantId });
    ...
    // Check if reservation status allows participant removal
    if (!["Pending", "Approved"].includes(reservation.status)) {
      return res.status(400).json({ 
        message: "Cannot remove participants from a reservation that is already ongoing, completed, or cancelled" 
      });
    }

    // Find participant index
    const participantIndex = reservation.participants.findIndex(
      p => p.id_number === participantId || p.idNumber === participantId
    );
    ...
    // ✅ Send notification to main reserver
    await notificationService.createNotification(
      {
