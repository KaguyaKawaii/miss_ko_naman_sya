...
// Add this method to your reservation controller
const removeParticipant = async (req, res) => {
  try {
    const { id } = req.params;
    const { participantIdNumber, isSelfRemoval = false } = req.body;
    ...
    // Check if participant exists in the reservation
    ...
    const isMainReserver = reservation.userId.toString() === userId.toString();
    const isSelfRemovalAttempt = participant.id_number === req.user.id_number;

    // Only allow removal for Pending or Approved reservations
    ...
    if (isSelfRemovalAttempt) {
      // User is trying to remove themselves - always allow
      if (!isSelfRemoval) {
        return res.status(400).json({ 
          message: "Self-removal flag required for leaving reservation" 
        });
      }
    } else {
      // User is trying to remove someone else
      if (!isMainReserver && userRole !== "Staff" && userRole !== "Admin") {
        return res.status(403).json({ 
          message: "Only the main reserver or staff can remove other participants" 
        });
      }
    }
    ...
    if (isSelfRemovalAttempt) {
      ...
      await sendNotification({
        to: reservation.userId, // Main reserver
        type: "participant_left",
        title: "Participant Left Reservation",
        message: `${participant.name} (${participant.id_number}) has left your reservation for ${reservation.roomName} on ${new Date(reservation.datetime).toLocaleDateString()}`,
      ...
      await sendNotification({
        to: userId,
        type: "left_reservation",
        title: "You Left Reservation",
        message: `You have successfully left the reservation for ${reservation.roomName} on ${new Date(reservation.datetime).toLocaleDateString()}`,
        relatedReservation: reservation._id
      });
    } else {
      // Notify the removed participant
      await sendNotification({
        to: participant._id, // This would need to be the actual user ID of the participant
        type: "removed_from_reservation",
        title: "Removed from Reservation",
        message: `You have been removed from the reservation for ${reservation.roomName} on ${new Date(reservation.datetime).toLocaleDateString()} by the main reserver`,
        relatedReservation: reservation._id
      });

      // Notify main reserver about the removal
      if (!isMainReserver) {
        await sendNotification({
          to: reservation.userId,
          type: "participant_removed",
          title: "Participant Removed",
          message: `${participant.name} (${participant.id_number}) has been removed from your reservation by staff`,
          relatedReservation: reservation._id
        });
      }
    }
    ...
    res.status(200).json({
      message: isSelfRemovalAttempt ? 
        "Successfully left the reservation" : 
