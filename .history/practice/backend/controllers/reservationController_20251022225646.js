...
/* ------------------------------------------------
   ✅ REMOVE PARTICIPANT (SELF-REMOVAL & MAIN RESERVER REMOVAL)
------------------------------------------------ */
exports.removeParticipant = async (req, res) => {
  try {
    const { id, participantId } = req.params;
    const { isSelfRemoval = false } = req.body; // Optional flag for self-removal
    const currentUser = req.user; // Assuming user is attached to request via auth middleware
    ...
    // Check if reservation allows participant changes
    if (!["Pending", "Approved"].includes(reservation.status)) {
      return res.status(400).json({ 
        message: "Participants can only be removed from Pending or Approved reservations." 
      });
    }

    // Find participant in reservation
    ...
    const participant = reservation.participants[participantIndex];

    // Authorization checks
    const isMainReserver = reservation.userId._id.toString() === currentUser._id.toString();
    const isSelfRemovalAttempt = participant.id_number === currentUser.id_number;
    const isStaffOrAdmin = currentUser.role === "Staff" || currentUser.role === "Admin";

    console.log('🔐 Authorization check:', {
      isMainReserver,
      isSelfRemovalAttempt,
      isStaffOrAdmin,
      participantIdNumber: participant.id_number,
      currentUserIdNumber: currentUser.id_number
    });

    // Check permissions
    if (isSelfRemovalAttempt) {
      // User is removing themselves - always allow
      console.log('✅ Self-removal authorized');
    } else if (isMainReserver || isStaffOrAdmin) {
      // Main reserver or staff removing someone else - allow
      console.log('✅ Main reserver/staff removal authorized');
    } else {
      // Unauthorized attempt to remove someone else
      console.log('❌ Unauthorized removal attempt');
      return res.status(403).json({ 
        message: "You can only remove yourself from this reservation." 
      });
    }

    // Remove the participant
    const removedParticipant = reservation.participants.splice(participantIndex, 1)[0];
    
    // Update number of users
    reservation.numUsers = reservation.participants.length + 1; // +1 for main reserver
    ...
    console.log('✅ Participant removed successfully:', {
      ...
      remainingParticipants: reservation.participants.length,
      newNumUsers: reservation.numUsers
    });
    ...
    try {
      await logAction(
        ...
        currentUser.name,
        isSelfRemovalAttempt ? "Left Reservation" : "Removed Participant",
        `${isSelfRemovalAttempt ? "Left" : "Removed ${removedParticipant.name} from"} reservation for ${reservation.roomName}`
      );
    } catch (logError) {
      console.warn("⚠️ Failed to log participant removal:", logError.message);
    }
    ...
    if (isSelfRemovalAttempt) {
      // Notify main reserver that a participant left
      await notificationService.createNotification(
        {
          userId: reservation.userId._id,
          reservationId: reservation._id,
      ...
      // Notify the participant who left
      await notificationService.createNotification(
        {
      ...
      // Notify main reserver (if removal was done by staff)
      if (!isMainReserver) {
        await notificationService.createNotification(
          {
            userId: reservation.userId._id,
    ...
    res.status(200).json({
      message: isSelfRemovalAttempt ? 
        "You have successfully left the reservation." : 
...
/* ------------------------------------------------
   ✅ EMAIL TEMPLATE FOR PARTICIPANT REMOVAL
------------------------------------------------ */
const generateRemovalEmail = ({ toName, reservation, removedBy, formattedDate, time, roomName, location }) => {
  return `
    <!DOCTYPE html>
    <html>
    <head>
        <style>
            ...
            .info-box { background: white; padding: 15px; border-radius: 5px; border-left: 4px solid #CC0000; margin: 15px 0; }
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>Study Room Reservation Update</h1>
            </div>
            <div class="content">
                <p>Dear <strong>${toName}</strong>,</p>
                
                <p>You have been removed from the following study room reservation:</p>
                ...
                <p>If this was a mistake, please contact ${removedBy} directly.</p>
                
                <p>Best regards,<br>University Study Room Reservation System</p>
            </div>
            <div class="footer">
