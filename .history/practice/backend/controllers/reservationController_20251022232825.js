...
exports.removeParticipant = async (req, res) => {
  try {
    const { reservationId } = req.params;
    const { participantIdNumber, reason } = req.body;
    ...
    if (["Ongoing", "Completed", "Cancelled", "Expired"].includes(reservation.status)) {
      return res.status(400).json({ 
        message: "Cannot modify participants for reservations that are ongoing, completed, cancelled, or expired" 
      });
    }
    ...
    reservation.numUsers = reservation.participants.length + 1; // +1 for main user
    ...
    await logAction(
      ...
      "System",
      "Participant Removed",
      `Removed ${removedParticipant.name} (${removedParticipant.id_number}) from reservation for ${reservation.roomName}. Reason: ${reason || "Not specified"}`
    );
        ...
        await sendEmail({
          ...
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h2 style="color: #CC0000;">You've Been Removed from a Reservation</h2>
              <p>Dear ${removedUser.name},</p>
              <p>You have been removed from the following reservation:</p>
              <div style="background: #f5f5f5; padding: 15px; border-radius: 5px; margin: 15px 0;">
                <p><strong>Room:</strong> ${reservation.roomName}</p>
                <p><strong>Date:</strong> ${reservation.date}</p>
                <p><strong>Time:</strong> ${new Date(reservation.datetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                <p><strong>Location:</strong> ${reservation.location}</p>
                ${reason ? `<p><strong>Reason:</strong> ${reason}</p>` : ''}
              </div>
              <p>If you believe this was a mistake, please contact the main reserver.</p>
            </div>
          `
        });
      } catch (emailError) {
    ...
    res.json({
      ...
      reservation: await Reservation.findById(reservationId).populate("userId")
    });
...
exports.addParticipant = async (req, res) => {
  try {
    const { reservationId } = req.params;
    const { participantIdNumber } = req.body;

    console.log('🔄 Adding participant to reservation:', {
      reservationId,
      participantIdNumber
    });
    ...
    if (reservation.participants.length >= 7) { // 7 participants + 1 main user = 8 total
      return res.status(400).json({ 
    ...
    if (newParticipantUser.id_number === mainUser.id_number) {
      return res.status(400).json({ message: "User is the main reserver of this reservation" });
    }

    // ✅ FLOOR ACCESS VALIDATION
    if (!validateFloorAccess(newParticipantUser, reservation.location)) {
      return res.status(400).json({
        message: `User ${newParticipantUser.name} does not have access to ${reservation.location}. ${getFloorRestrictionMessage(reservation.location)}`
      });
    }
    ...
    if (conflicts.length > 0) {
      return res.status(400).json({ 
        message: `User ${newParticipantUser.name} has a conflicting reservation during this time` 
      });
    }
    ...
    await logAction(
      ...
      "System",
      "Participant Added",
      `Added ${newParticipantUser.name} (${newParticipantUser.id_number}) to reservation for ${reservation.roomName}`
    );
    ...
    await notificationService.createNotification(
      {
        ...
        date: reservation.date,
        startTime: new Date(reservation.datetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        mainReserverName: mainUser.name
      },
      req.app.get("io")
    );
    ...
    try {
      ...
      console.warn('⚠️ Failed to send addition email:', emailError.message);
    }
