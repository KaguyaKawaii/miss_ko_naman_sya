...
/* ------------------------------------------------
   ✅ GET PARTICIPANTS WITH DETAILS FOR MANAGEMENT
------------------------------------------------ */
...
exports.removeParticipant = async (req, res) => {
  try {
    ...
    const { userId } = req.body; // Main reserver's user ID for verification

    if (!reservationId || !participantId || !userId) {
      return res.status(400).json({ message: "Reservation ID, participant ID, and user ID are required" });
    }
    ...
    // Verify the requesting user is the main reserver
    if (reservation.userId.toString() !== userId) {
      return res.status(403).json({ message: "Only the main reserver can remove participants" });
    }

    // Check if reservation status allows modifications
    if (!["Pending", "Approved"].includes(reservation.status)) {
      return res.status(400).json({ message: "Cannot modify participants for reservations that are not Pending or Approved" });
    }
    ...
    // Get participant details before removal for notification
    ...
    const removedUser = await User.findOne({ id_number: participantId });
    if (removedUser) {
      await notificationService.createNotification(
        {
      ...
      try {
        ...
        console.warn('⚠️ Failed to send removal email to participant:', emailError.message);
      }
    }
...
exports.addParticipant = async (req, res) => {
  try {
    const { reservationId } = req.params;
    const { userId, participantIdNumber } = req.body;

    if (!reservationId || !userId || !participantIdNumber) {
      return res.status(400).json({ message: "Reservation ID, user ID, and participant ID number are required" });
    }
    ...
    // Verify the requesting user is the main reserver
    if (reservation.userId.toString() !== userId) {
      return res.status(403).json({ message: "Only the main reserver can add participants" });
    }
    ...
    // Check group size limit (4-8 including main user)
    if (reservation.participants.length >= 7) { // 7 participants + 1 main user = 8
      return res.status(400).json({ message: "Maximum group size of 8 users reached" });
    }

    // Find the user to add as participant
    const newParticipantUser = await User.findOne({ 
      id_number: participantIdNumber.toString().trim() 
    });
    ...
    if (isAlreadyParticipant) {
      return res.status(400).json({ message: "User is already a participant in this reservation" });
    }

    // Check if user is the main reserver
    if (newParticipantUser.id_number === reservation.userId.id_number) {
      return res.status(400).json({ message: "Cannot add main reserver as participant" });
    }
    ...
    if (conflictingReservations.length > 0) {
      return res.status(400).json({ message: `Participant ${newParticipantUser.name} has a conflicting reservation during this time.` });
    }
    ...
    await notificationService.createNotification(
      {
        userId: newParticipantUser._id,
        message: `You have been added as a participant to a reservation for ${reservation.roomName} on ${reservation.date}`,
        reservationId: reservation._id,
        type: "reservation",
        status: "participant_added",
        targetRole: "user",
    ...
    try {
      await sendEmail({
        ...
        html: generateReservationEmail({
          ...
          participants: reservation.participants,
          isParticipant: true,
          extraNote: "You have been added to this reservation as a participant."
        })
      });
    } catch (emailError) {
      console.warn('⚠️ Failed to send addition email to participant:', emailError.message);
    }
...
/* ------------------------------------------------
   ✅ VALIDATE PARTICIPANT ID NUMBER
------------------------------------------------ */
exports.validateParticipantId = async (req, res) => {
  try {
    ...
    // If reservationId is provided, check if user is already a participant
    if (reservationId) {
      const reservation = await Reservation.findById(reservationId);
      if (reservation) {
        const isAlreadyParticipant = reservation.participants.some(
        ...
        // Check if user is the main reserver
        if (user.id_number === reservation.userId.id_number) {
          return res.json({ 
            valid: false, 
            message: "Cannot add main reserver as participant" 
          });
        }
        ...
        if (!validateFloorAccess(user, reservation.location)) {
          return res.json({ 
            valid: false, 
            message: `User does not have access to ${reservation.location}` 
          });
        }
      }
    }
    ...
    res.status(500).json({ valid: false, message: "Error validating ID number" });
  }
};