...
/* ------------------------------------------------
   ✅ UPDATE RESERVATION PARTICIPANTS
------------------------------------------------ */
exports.updateReservationParticipants = async (req, res) => {
  try {
    const { id } = req.params;
    const { participants, userId } = req.body; // userId passed in request body

    console.log('🔄 Updating participants for reservation:', id);
    ...
    for (const participant of participants) {
      if (!participant.id_number || !participant.name) {
        return res.status(400).json({ 
          message: 'Each participant must have id_number and name' 
        });
      }
    }

    // Check total group size (including main user)
    const totalGroupSize = participants.length + 1;
    if (totalGroupSize < 4 || totalGroupSize > 8) {
      return res.status(400).json({
        message: 'Group size must be between 4 and 8 users including the main reserver'
      });
    ...
    // Validate floor access for new participants
    const invalidParticipants = [];
    const enrichedParticipants = [];
    ...
    for (const participant of participants) {
      ...
      if (!participantUser) {
        return res.status(400).json({ 
          message: `Participant ${participant.name} (ID: ${participant.id_number}) not found` 
        });
      }
    ...
    if (invalidParticipants.length > 0) {
      const invalidNames = invalidParticipants.map(p => p.name).join(', ');
      return res.status(400).json({
        message: `The following participants do not have access to ${reservation.location}: ${invalidNames}. ${getFloorRestrictionMessage(reservation.location)}`,
        invalidParticipants: invalidParticipants
      });
    }
    ...
    console.log('✅ Participants updated successfully for reservation:', id);
