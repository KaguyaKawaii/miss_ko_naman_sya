...
/* ------------------------------------------------
   ✅ CREATE RESERVATION (WITH FLOOR ACCESS VALIDATION) - FIXED PARTICIPANT ID FETCHING
------------------------------------------------ */
exports.createReservation = async (req, res) => {
  try {
    ...
    // ✅ FIXED: Validate participants & collect emails + FLOOR ACCESS VALIDATION
    ...
    if (participants && participants.length > 0) {
      for (const participant of participants) {
        ...
        if (!participantUser) {
          console.log('❌ Participant not found with ID:', participant.id_number);
          return res.status(400).json({ 
            message: `Participant with ID ${participant.id_number} not found in database.` 
          });
        }

        console.log('✅ Participant user details:', {
          name: participantUser.name,
          id_number: participantUser.id_number,
          department: participantUser.department,
          course: participantUser.course,
        ...
        if (!participantUser.verified) {
          console.log('❌ Participant not verified:', participantUser.name);
          return res.status(400).json({ 
