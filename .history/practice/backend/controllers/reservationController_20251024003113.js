...
/* ------------------------------------------------
   ✅ CREATE RESERVATION (WITH FLOOR ACCESS VALIDATION) - FIXED FOR NO PARTICIPANTS
------------------------------------------------ */
exports.createReservation = async (req, res) => {
  try {
    ...
    // FIXED: Handle case where participants might be undefined or empty
    ...
    // FIXED: Only validate participants if they exist
    ...
    const reservation = await Reservation.create({
      ...
      location,
      roomName,
      participants: enrichedParticipants, // This will be empty array if no participants
      status: "Pending",
    ...
    // ✅ NOTIFY PARTICIPANTS - Only if there are participants
    ...
    // ✅ FIXED: Send email to participants - Only if there are participants
    if (enrichedParticipants.length > 0) {
      for (const participant of enrichedParticipants) {
        // Skip if this is the main user
...
exports.getParticipantsDetails = async (req, res) => {
  try {
    // Support both :id and :reservationId parameters
    const reservationId = req.params.id || req.params.reservationId;
    
    console.log('🔍 Fetching participants for reservation:', reservationId);
    ...
    console.log('📋 Found reservation:', reservation.roomName);
    console.log('👥 Participants count:', reservation.participants.length);

    // Get enriched participant details
    const enrichedParticipants = await Promise.all(
      reservation.participants.map(async (participant) => {
        console.log('🔍 Looking up participant:', participant.id_number);
    ...
    res.status(500).json({ 
      message: "Failed to fetch participants details",
...
/* ------------------------------------------------
   ✅ GET PARTICIPANTS FOR MANAGEMENT
------------------------------------------------ */
exports.getReservationParticipants = async (req, res) => {
  try {
    const { id } = req.params;
    
    console.log('🔍 Fetching participants for reservation:', id);
...
exports.addParticipant = async (req, res) => {
  try {
    ...
    console.log('➕ Adding participant to reservation:', { id, id_number });
    ...
    try {
      ...
      console.warn("⚠️ Failed to create notification for participant:", notifError.message);
    }
    ...
    try {
      ...
      console.warn("⚠️ Failed to send email to participant:", emailError.message);
    }
...
/* ------------------------------------------------
   ✅ REMOVE PARTICIPANT FROM RESERVATION (FIXED)
------------------------------------------------ */
exports.removeParticipant = async (req, res) => {
  try {
    ...
    if (!targetIdNumber) {
      console.log('❌ Missing participant ID in request body');
      return res.status(400).json({ 
        success: false,
    ...
    console.log('👥 Current participants:', reservation.participants.map(p => ({
      id: p.id_number,
      name: p.name
    })));
    ...
    if (participantIndex === -1) {
      console.log('❌ Participant not found:', targetIdNumber);
      console.log('🔍 Available participants:', reservation.participants.map(p => p.id_number));
      return res.status(404).json({ 
        success: false,
    ...
    console.log('✅ Participant removed:', removedParticipant.name);
...
exports.getReservationById = async (req, res) => {
  try {
    ...
    const response = {
      ...reservation,
      participants: enrichedParticipants,
      // ✅ Add these fields for easy access in frontend
      mainUser: {
        _id: reservation.userId?._id,
        id_number: reservation.userId?.id_number,
        name: reservation.userId?.name,
        email: reservation.userId?.email,
        course: reservation.userId?.course,
        year_level: reservation.userId?.yearLevel,
        department: reservation.userId?.department
      }
    };

    console.log('✅ Successfully fetched reservation:', reservation._id);
    console.log('👤 Main user data:', {
      userId: reservation.userId?._id,
      id_number: reservation.userId?.id_number,
      name: reservation.userId?.name
    });
