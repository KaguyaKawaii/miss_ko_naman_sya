...
// ✅ RULE: Validate that numUsers represents additional participants (excluding main reserver)
const additionalParticipantsCount = participants?.length || 0;

// Check if the frontend numUsers matches the actual additional participants count
if (additionalParticipantsCount !== numUsers) {
  return res.status(400).json({
    message: `Number of additional participants (${additionalParticipantsCount}) does not match the selected group size (${numUsers}).`
  });
}

// Validate numUsers is within allowed range (3-7 additional participants + 1 main = 4-8 total)
if (numUsers < 3 || numUsers > 7) {
  return res.status(400).json({
    message: "Number of additional participants must be between 3 and 7 (4-8 users total including main reserver)."
  });
}
