...
// ✅ FIXED: Handle both interpretations of numUsers (additional participants OR total group size)
const additionalParticipantsCount = participants?.length || 0;

// Check if numUsers represents total group size (including main) or additional participants only
let totalGroupSize;
let additionalParticipantsExpected;

if (numUsers >= 4 && numUsers <= 8) {
  // numUsers represents total group size (including main reserver)
  totalGroupSize = numUsers;
  additionalParticipantsExpected = numUsers - 1; // Subtract the main reserver
} else if (numUsers >= 3 && numUsers <= 7) {
  // numUsers represents additional participants only (legacy format)
  totalGroupSize = numUsers + 1;
  additionalParticipantsExpected = numUsers;
} else {
  return res.status(400).json({
    message: "Group size must be between 4 and 8 users total (including main reserver)."
  });
}

// Validate that the actual participants count matches expected
if (additionalParticipantsCount !== additionalParticipantsExpected) {
  return res.status(400).json({
    message: `Number of additional participants (${additionalParticipantsCount}) does not match the selected group size. Expected ${additionalParticipantsExpected} additional participants for a total of ${totalGroupSize} users.`
  });
}

// Calculate total group size for the reservation record
const totalGroupSize = numUsers + 1; // Additional participants + main reserver
...
exports.addParticipant = async (req, res) => {
  try {
    ...
    // Check maximum participants (4 participants + 1 main reserver = 5 total)
    const totalGroupSize = reservation.participants.length + 1; // +1 for main reserver
    if (totalGroupSize >= 5) {
      return res.status(400).json({ 
        message: "Maximum group size reached (4 participants + main reserver). Cannot add more participants." 
      });
    }
    ...
    // ✅ CREATE NOTIFICATION for new participant - FIXED TYPE
    await notificationService.createNotification(
      {
        userId: newParticipantUser._id,
        reservationId: reservation._id,
        type: "reservation", // CHANGED FROM "participant" TO "reservation"
        status: "participant_added", // Use a custom status to identify this specific case
        targetRole: "user",
        roomName: reservation.roomName,
        date: reservation.date,
        startTime: new Date(reservation.datetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        endTime: new Date(reservation.endDatetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        // Add custom message for participant addition
