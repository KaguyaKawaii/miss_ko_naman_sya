...
// Validate that additional participants count matches expected
const expectedAdditionalParticipants = totalGroupSize - 1;
if (additionalParticipantsCount !== expectedAdditionalParticipants) {
  return res.status(400).json({
    message: `Number of additional participants (${additionalParticipantsCount}) does not match the selected group size. Expected ${expectedAdditionalParticipants} additional participants for a total of ${totalGroupSize} users.`
  });
}
