...
const totalGroupSize = numUsers; // numUsers is total including main reserver
...
// ✅ FIXED: Remove the validation that checks exact participant count match
// This validation is too strict and causes issues when users don't fill all participant slots
// The frontend allows users to select 8 users but only fill some participant fields
if (additionalParticipantsCount > totalGroupSize - 1) {
  return res.status(400).json({
    message: `Number of additional participants (${additionalParticipantsCount}) exceeds the maximum allowed for the selected group size. Maximum additional participants for ${totalGroupSize} total users is ${totalGroupSize - 1}.`
  });
}
