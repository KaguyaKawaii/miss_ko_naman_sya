...
// ✅ FIXED: Validate that additional participants don't exceed maximum
// For totalGroupSize = 8, maximum additional participants is 7
const maxAdditionalParticipants = totalGroupSize - 1;
if (additionalParticipantsCount > maxAdditionalParticipants) {
  return res.status(400).json({
    message: `Too many additional participants. For ${totalGroupSize} total users, maximum additional participants is ${maxAdditionalParticipants}. You have ${additionalParticipantsCount}.`
  });
}

// ✅ FIXED: Validate minimum group size
const minAdditionalParticipants = 3; // 3 additional + 1 main = 4 minimum
if (additionalParticipantsCount < minAdditionalParticipants) {
  return res.status(400).json({
    message: `Not enough participants. Minimum ${minAdditionalParticipants} additional participants required for ${totalGroupSize} total users.`
  });
}
