...
console.log('🔍 Group size validation:', {
  ...
  expectedAdditional: totalGroupSize - 1
});
...
// ✅ FIXED: The participants array should contain exactly (totalGroupSize - 1) participants
// For 8 total users: you + 7 additional participants = 8
const expectedAdditionalParticipants = totalGroupSize - 1;

if (additionalParticipantsCount !== expectedAdditionalParticipants) {
  return res.status(400).json({
    message: `Participant count mismatch. For ${totalGroupSize} total users, expected ${expectedAdditionalParticipants} additional participants, but found ${additionalParticipantsCount}.`
  });
}

// Validate that all participants are filled and verified
for (let i = 0; i < participants.length; i++) {
  const participant = participants[i];
  if (!participant.name || !participant.id_number) {
    return res.status(400).json({
      message: `Please complete all fields for participant ${i + 1}.`
    });
  }
}
