...
exports.createReservation = async (req, res) => {
  try {
    ...
    if (totalParticipantsCount !== totalGroupSize) {
      return res.status(400).json({
        message: `Participant count mismatch. Expected ${totalGroupSize} total participants (including main reserver), but found ${totalParticipantsCount}.`
      });
    }

    // ✅ FIXED: Check that the main reserver is included in the participants array (NOW AFTER user is fetched)
