...
function AdminDashboard({ setView }) {
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      const endpoints = [
        ...
        { key: 'availableRooms', url: '/api/rooms/available' },
  ...
  const processFetchedData = (data) => {
    ...
    const allRoomsData = data.rooms?.error ? [] : data.rooms;
    const availableRoomsData = data.availableRooms?.error ? [] : data.availableRooms;
    const reportsData = data.reports?.error ? [] : data.reports;

    // Calculate room statistics
    const totalRooms = safeLength(allRoomsData);
    const availableRoomsCount = safeLength(availableRoomsData);
  ...
  const getReservationCompletionRate = () => {
    const totalReservations = summaryData.reservations;
    const completedReservations = reservations.filter(r => 
      ['Completed', 'Expired'].includes(r.status)
    ).length;
    return totalReservations > 0 ? Math.round((completedReservations / totalReservations) * 100) : 0;
  };

  const getRoomUtilizationRate = () => {
    return summaryData.rooms > 0 ? Math.round((summaryData.occupiedRooms / summaryData.rooms) * 100) : 0;
  };

  const getReportResolutionRate = () => {
    const totalReports = summaryData.reports;
    const resolvedReports = reports.filter(r => 
      ['Resolved', 'Archived'].includes(r.status)
    ).length;
    return totalReports > 0 ? Math.round((resolvedReports / totalReports) * 100) : 0;
  };
  ...
  const statCards = [
    {
      ...
      value: summaryData.reservations,
      icon: (
      ...
      color: 'blue',
      subtitle: `${summaryData.pendingReservations} pending approval`,
      trend: '+12% this week',
      percentage: getReservationCompletionRate(),
    ...
    },
    {
      ...
      color: 'green',
      subtitle: `${getNewUsersThisWeek()} new this week`,
      trend: '+5% this month',
      percentage: Math.round((getNewUsersThisWeek() / summaryData.users) * 100) || 0,
      percentageLabel: 'Growth Rate'
    },
    {
      id: 'rooms',
      title: 'Room Status',
      value: summaryData.rooms,
      icon: (
        <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
      ...
      color: 'purple',
      subtitle: `${summaryData.availableRooms} available, ${summaryData.occupiedRooms} occupied`,
      trend: `${getRoomUtilizationRate()}% utilized`,
      percentage: getRoomUtilizationRate(),
      percentageLabel: 'Utilization Rate'
    },
    {
      id: 'reports',
      title: 'Reports & Issues',
      value: summaryData.reports,
      icon: (
        <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
      ...
      color: 'amber',
      subtitle: `${summaryData.pendingReports} pending resolution`,
      trend: getReportResolutionRate() + '% resolved',
      percentage: getReportResolutionRate(),
      percentageLabel: 'Resolution Rate'
    }
  ];
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {statCards.map((card) => (
            <div 
              key={card.id}
              className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 group relative overflow-hidden"
            >
                ...
                {card.percentage > 0 && (
                  <div className="mt-4">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-xs text-gray-500">{card.percentageLabel}</span>
                      <span className="text-xs font-semibold text-gray-700">{card.percentage}%</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className={`bg-${card.color}-500 h-2 rounded-full transition-all duration-500 ease-out`}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white p-4 rounded-lg border border-gray-200 text-center">
            <div className="text-2xl font-bold text-blue-600">{reservations.filter(r => r.status === 'Ongoing').length}</div>
            <div className="text-xs text-gray-500 mt-1">Active Now</div>
          </div>
          <div className="bg-white p-4 rounded-lg border border-gray-200 text-center">
            <div className="text-2xl font-bold text-green-600">{usersData?.filter(u => u.verified).length || 0}</div>
            <div className="text-xs text-gray-500 mt-1">Verified Users</div>
          </div>
          <div className="bg-white p-4 rounded-lg border border-gray-200 text-center">
            <div className="text-2xl font-bold text-purple-600">{Math.round(getRoomUtilizationRate())}%</div>
            <div className="text-xs text-gray-500 mt-1">Room Usage</div>
          </div>
          <div className="bg-white p-4 rounded-lg border border-gray-200 text-center">
            <div className="text-2xl font-bold text-amber-600">{reports.filter(r => r.status === 'In Progress').length}</div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl">
            <div className="p-6 border-b border-gray-200">
              <h2 className="text-xl font-bold text-gray-800">Post New Announcement</h2>
            </div>
