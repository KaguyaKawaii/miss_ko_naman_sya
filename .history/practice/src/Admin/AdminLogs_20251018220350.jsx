...
function AdminLogs({ setView }) {
  ...
  <div className="max-h-[65vh] overflow-y-auto p-4">
    <div className="space-y-2">
      {filteredLogs.map((log) => (
        <div key={log._id} className="flex items-center gap-4 hover:bg-gray-800 px-4 py-3 rounded-lg transition-colors border-l-4 border-blue-500">
          {/* Time */}
          <div className="text-cyan-400 text-xs font-mono min-w-[85px] flex-shrink-0">
            {new Date(log.createdAt).toLocaleTimeString("en-PH", {
              hour: '2-digit',
              minute: '2-digit',
              second: '2-digit'
          ...
          <div className="min-w-[140px]">
            <span className="inline-flex px-3 py-1 rounded-full text-xs font-bold bg-green-900 bg-opacity-80 text-green-300 border border-green-700">
              {log.action}
            </span>
          </div>
          ...
          {/* Full Date */}
          <div className="text-gray-500 text-xs min-w-[90px] text-right flex-shrink-0 font-mono">
            {new Date(log.createdAt).toLocaleDateString("en-PH", {
              month: 'short',
              day: '2-digit',
              year: 'numeric'
