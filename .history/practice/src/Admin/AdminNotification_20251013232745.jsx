// AdminNotification.jsx - FIXED VERSION
...
const AdminReservationModal = ({ 
  ...
  // FIXED: Create participants array with proper fallbacks
  const mainReserver = {
  ...
  // FIXED: Handle participants array properly with fallbacks
  const additionalParticipants = (reservation.participants || []).slice(0, 3).map(participant => ({
...
// Main AdminNotification Component - FIXED VERSION
function AdminNotification({ setView }) {
  ...
  // FIXED: Enhanced openReservationModal function
  const openReservationModal = async (notification) => {
    try {
      ...
      if (reservationId) {
        // FIXED: Use the populated reservation endpoint
        const reservationRes = await axios.get(`http://localhost:5000/reservations/${reservationId}?populate=true`);
  ...
  // FIXED: Enhanced transformReservationData function
  const transformReservationData = (reservation) => {
    if (!reservation) return null;

    console.log("Transforming reservation data:", reservation);

    // FIXED: Better handling of user data with fallbacks
    const mainReserver = {
      name: reservation.userId?.name || "Unknown User",
      id_number: reservation.userId?.id_number || "N/A",
      course: reservation.userId?.course || "N/A",
    ...
    // FIXED: Handle participants array with proper fallbacks
    const additionalParticipants = (reservation.participants || []).slice(0, 3).map(participant => ({
      name: participant.name || "Unknown User",
      id_number: participant.id_number || "N/A",
      course: participant.course || "N/A",
  ...
  // FIXED: Enhanced getTotalParticipants function
  const getTotalParticipants = (notification) => {
    ...
    // Limit to maximum 4 participants total (main reserver + 3 additional)
    return Math.min(hasMainReserver + additionalParticipants, 4);
  };

  // FIXED: Enhanced formatAdminMessage function
  ...
  // Keep the rest of your existing functions (markAsRead, handleNotificationClick, etc.)
