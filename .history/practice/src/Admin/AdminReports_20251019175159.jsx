...
import { Eye, RefreshCw, Search, ChevronDown, X, Trash2 } from "lucide-react";
import {
  Clock,
  MapPin,
  Calendar,
...
const fetchReports = () => {
  ...
  const handleArchiveFromTable = async (reportId, reportCategory) => {
    if (!window.confirm(`Are you sure you want to archive the report "${reportCategory}"?`)) {
      return;
    }
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/reports/${reportId}/archive`, {
                      ...
                      <tr key={report._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <div className="flex justify-end space-x-2">
                            ...
                            {/* Archive (Trash Can) - Only show for non-archived reports */}
                            {report.status !== "Archived" && (
                              <button
                                onClick={() => handleArchiveFromTable(report._id, report.category)}
