...
// ✅ Also fix the fetch URL for archived reports
const fetchArchivedReports = async () => {
  try {
    setLoading(true);
    const res = await axios.get("http://localhost:5000/api/reports/archived"); // Fixed URL
...
// ✅ Restore report - FIXED
const handleRestore = async (id) => {
  try {
    // Get current user ID from your auth context or localStorage
    const currentUser = JSON.parse(localStorage.getItem("user")); // Adjust based on your auth setup
...
// ✅ Delete report permanently - FIXED
const handleDelete = async (id) => {
  try {
    ...
    await axios.delete(`http://localhost:5000/api/reports/${id}`, {
      data: { deletedBy: deletedBy } // Note: for DELETE requests, use data property
    });
