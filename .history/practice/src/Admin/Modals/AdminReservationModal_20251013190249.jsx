...
const AdminReservationModal = ({ 
  ...
  const InfoCard = ({ title, value, icon, color = "gray" }) => (
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[95vh] overflow-hidden border border-gray-300">
        {/* Header */}
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-6 border-b border-slate-700">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white mb-1">{reservation.roomName}</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={onClose}
                disabled={isProcessing}
                className="p-2 text-white/80 hover:text-white hover:bg-white/10 rounded-lg transition-all duration-200 disabled:opacity-50"
              >
                <X size={24} />
              </button>
            </div>
          ...
          {activeTab === "overview" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <InfoCard
                  title="Duration"
                  value={`${Math.round((currentEndTime - new Date(reservation.datetime)) / (1000 * 60 * 60))}h`}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-5">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900">Main Reserver</h3>
                  </div>
                  <div className="space-y-3">
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <div>
                        <p className="font-semibold text-gray-900">{reservation.userId?.name || "N/A"}</p>
                        <p className="text-sm text-gray-600">{reservation.userId?.id_number || "N/A"}</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div className="flex items-center gap-2 text-gray-600">
                        <Shield size={14} />
                        <span>{reservation.userId?.department || "N/A"}</span>
                      </div>
                      <div className="flex items-center gap-2 text-gray-600">
                        <Tag size={14} />
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{participantCount} additional members</p>
                    </div>
                  </div>
                  <div className="px-3 py-1.5 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                    Total: {displayUserCount} users
                  </div>
                </div>
                  ...
                  {reservation.participants && reservation.participants.length > 0 ? (
                    reservation.participants.map((participant, index) => (
                      <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200 hover:bg-gray-100 transition-colors duration-200">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-gradient-to-br from-indigo-400 to-indigo-500 rounded-full flex items-center justify-center text-white font-semibold text-sm">
                            {participant.name.charAt(0)}
                          </div>
                          <div>
                            <p className="font-semibold text-gray-900">{participant.name}</p>
                            <p className="text-sm text-gray-600">{participant.idNumber} · {participant.department}</p>
                          </div>
                        </div>
                    ...
                    <div className="text-center py-12 text-gray-500">
                      ...
                      <p className="text-lg font-medium text-gray-400">No additional participants</p>
                      <p className="text-sm">Only the main reserver is attending this session</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      {showExtensionModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-[60] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-gray-300">
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-t-2xl text-white">
              <div className="flex items-center gap-3">
