...
const AdminReservationModal = ({ 
  ...
  const handleAction = async (action, data = {}) => {
    ...
    try {
      ...
      switch (action) {
        case "approve":
          endpoint = `http://localhost:5000/api/reservations/${reservation._id}/status`;
          method = "patch";
        ...
        case "start":
          endpoint = `http://localhost:5000/api/reservations/start/${reservation._id}`;
          method = "post";
          break;

        case "end-early":
  endpoint = `http://localhost:5000/api/reservations/${reservation._id}/end-early`;
  method = "post";
  break;

        case "cancel":
          endpoint = `http://localhost:5000/api/reservations/${reservation._id}`;
          method = "delete";
          break;

        case "request-extension":
          endpoint = `http://localhost:5000/api/reservations/${reservation._id}/request-extension`;
          method = "put";
          requestData = { 
        ...
        case "approve-extension":
          endpoint = `http://localhost:5000/api/reservations/${reservation._id}/handle-extension`;
          method = "put";
          ...
          <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
            {["overview", "participants", "details"].map((tab) => (
              <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 flex-1 cursor-pointer hover:bg-gray-200/50 ${
                  activeTab === tab
