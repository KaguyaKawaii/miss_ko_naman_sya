...
const ReportModal = ({ reportId, onClose, onReportUpdated }) => {
  ...
  // 🔹 Fetch staff list - Only staff assigned to the same floor as the report
  useEffect(() => {
    const fetchStaff = async () => {
      ...
      try {
        ...
        const allStaff = res.data.users || [];
        
        // Filter staff by floor - only show staff assigned to the same floor as the report
        const filteredStaff = allStaff.filter(staff => 
          staff.floor === report.floor || !staff.floor // Include staff without floor assignment
        );
                      ...
                      <div className="space-y-4">
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">
                            Select Staff Member ({report.floor} Floor)
                          </label>
                          <select
                            value={selectedStaff}
                          ...
                          {staffList.length === 0 && (
                            <p className="text-sm text-amber-600 mt-2">
                              No staff members available for {report.floor} floor.
                            </p>
                          )}
                        </div>
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-5">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <h3 className="text-lg font-semibold text-gray-900">
                        {report.status === "Resolved" ? "Action Taken" : "Resolution Details"}
                      </h3>
                    </div>
