...
const ReportModal = ({ reportId, onClose, onReportUpdated }) => {
  ...
  // 🔹 Fetch all staff list - Include staff from all floors
  useEffect(() => {
    const fetchStaff = async () => {
      try {
        ...
        try {
          // First try: Get all users and filter by role
          ...
          console.error("Error with first API endpoint:", err);
          
          // Second try: Direct staff endpoint
          try {
            const res = await axios.get(`${USERS_API}/staff`);
            console.log("Staff endpoint response:", res.data);
            
            if (res.data && Array.isArray(res.data)) {
              staffData = res.data;
            } else if (res.data && Array.isArray(res.data.users)) {
              staffData = res.data.users;
            }
          } catch (err2) {
            console.error("Error with staff endpoint:", err2);
          }
        }
        ...
        if (sortedStaff.length === 0) {
          console.warn("No staff members found. Check API response structure.");
        }
      } catch (err) {
                          ...
                          <div>
                            ...
                            {staffList.length === 0 && (
                              <p className="text-sm text-amber-600 mt-2">
                                No staff members available. Please check if staff users exist in the system.
                              </p>
                            )}
                          </div>
