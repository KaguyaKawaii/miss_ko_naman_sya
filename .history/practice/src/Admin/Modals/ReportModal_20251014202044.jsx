...
const ReportModal = ({ reportId, onClose, onReportUpdated }) => {
  ...
  // 🔹 Fetch all staff list - Fixed API endpoint
  useEffect(() => {
    const fetchStaff = async () => {
      try {
        console.log("Fetching staff list from:", USERS_API);
        
        // Use the main users endpoint
        ...
        if (res.data.success && Array.isArray(res.data.users)) {
          // If response has success property and users array
        ...
        if (sortedStaff.length === 0) {
          console.warn("No staff members found. Available users:", res.data);
        }
      } catch (err) {
        ...
        try {
          ...
          const altRes = await axios.get("http://localhost:5000/api/staff");
          console.log("Alternative endpoint response:", altRes.data);
          
          let altStaffData = [];
          if (altRes.data.success && Array.isArray(altRes.data.staff)) {
            altStaffData = altRes.data.staff;
          } else if (Array.isArray(altRes.data)) {
            altStaffData = altRes.data;
          }
          setStaffList(altStaffData);
        } catch (altErr) {
  ...
  // Group staff by floor for better organization
  const groupStaffByFloor = () => {
    const grouped = {};
    
    staffList.forEach(staff => {
      const floor = staff.floor || 'Unassigned Floor';
      if (!grouped[floor]) {
        grouped[floor] = [];
      }
                        ...
                        <div className="space-y-4">
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                              Select Staff Member (All Floors)
                            </label>
                            <select
                              value={selectedStaff}
                                ...
                                <optgroup key={group.floor} label={`${group.floor}`}>
                                  {group.staff.map((staff) => (
                                    <option key={staff._id} value={staff._id}>
                                      {staff.name} - {staff.floor || 'No Floor Assigned'}
                                    </option>
                                  ))}
                                </optgroup>
                              ))}
                            ...
                            {staffList.length === 0 && (
                              <div className="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                                <p className="text-amber-700 text-sm">
                                  No staff members found. Please check:
                                </p>
                                <ul className="text-amber-600 text-xs mt-1 list-disc list-inside">
                                  <li>API endpoint: {USERS_API}</li>
                                  <li>Staff users exist in database with role: "Staff"</li>
