...
const ReportModal = ({ reportId, onClose, onReportUpdated }) => {
  ...
  // 🔹 Fetch all staff list - Enhanced with multiple endpoint attempts
  useEffect(() => {
    const fetchStaff = async () => {
      try {
        ...
        if (sortedStaff.length === 0) {
          console.warn("No staff members found after trying all endpoints");
        }
      } catch (err) {
  ...
  const groupStaffByFloor = () => {
    ...
    staffList.forEach(staff => {
      const floor = staff.floor || 'All Floors';
      if (!grouped[floor]) {
        grouped[floor] = [];
      }
                                ...
                                <optgroup key={group.floor} label={`${group.floor}`}>
                                  {group.staff.map((staff) => (
                                    <option key={staff._id} value={staff._id}>
                                      {staff.name} - {staff.floor || 'All Floors'}
                                    </option>
                                  ))}
                                </optgroup>
                              ))}
