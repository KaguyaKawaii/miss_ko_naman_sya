...
const ReportModal = ({ reportId, onClose, onReportUpdated }) => {
  ...
  useEffect(() => {
    const fetchStaff = async () => {
      try {
        const res = await axios.get("http://localhost:5000/users?role=Staff");
        let staffData = [];
        ...
        const sortedStaff = staffWithFloors.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
  ...
  // 🔹 Resolve Report - FIXED: Don't send invalid user IDs
  const handleResolveReport = async () => {
    ...
    try {
      ...
      // FIXED: Only send actionTaken, don't send invalid user IDs
      const response = await axios.put(`${API_URL}/resolve/${report._id}`, {
        actionTaken: actionTaken.trim()
        // Don't send resolvedBy unless you have a valid user ID from authentication
      });
  ...
  // 🔹 Archive Report - FIXED: Don't send invalid user IDs
  const handleArchiveReport = async () => {
    if (!report) return;

    try {
      setActionLoading(true);
      // FIXED: Don't send archivedBy unless you have a valid user ID
