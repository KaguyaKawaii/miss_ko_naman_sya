...
function App() {
  ...
  /* ---------- FORCE LOGOUT WHEN MAINTENANCE MODE ACTIVATES ---------- */
  const forceLogoutDuringMaintenance = () => {
    if (user && maintenanceMode) {
      console.log("Maintenance mode activated - forcing logout for user:", user.role);
      localStorage.clear();
      setUser(null);
...
// In the maintenance mode check function, update this part:
const handleMaintenanceRedirect = (maintenanceData) => {
  ...
  // Force logout all non-admin users during maintenance
  if (maintenanceData.maintenanceMode && user && user.role !== 'admin') {
    forceLogoutDuringMaintenance();
    return;
  }
  ...
  // ALWAYS allow admin login page during maintenance - remove the allowAdminAccess check for adminLogin
  const canAccess = (isAdmin && maintenanceData.allowAdminAccess) || 
                   isAdminLogin || // Always allow admin login page
                   isMaintenancePage;
  ...
  if (!canAccess && !isMaintenancePage) {
    ...
    console.log("Access allowed. User:", user?.role, "Admin access allowed:", maintenanceData.allowAdminAccess, "View:", view);
  }
};

// Update the isViewAllowed function:
const isViewAllowed = () => {
  ...
  // ALWAYS allow admin login page during maintenance
  return (isAdmin && allowAdminAccess) || 
         isAdminLogin || // Always allow admin login
         isMaintenancePage;
};
  ...
  useEffect(() => {
    ...
    socket.on('maintenance-mode-updated', (data) => {
      ...
      // Force logout when maintenance mode is activated
  ...
  const checkMaintenanceMode = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        // Force logout if maintenance mode is active and user is not admin
  ...
  const handleLoginSuccess = (userData) => {
    // Block login during maintenance for non-admin users
  ...
  const handleAdminLoginSuccess = (adminData) => {
    // Block admin login during maintenance if admin access is not allowed
    if (maintenanceMode && !allowAdminAccess) {
      alert("Admin access is currently disabled during maintenance.");
      return;
    }
  ...
  const isViewAllowed = () => {
    ...
    // Only allow admin login and maintenance pages during maintenance
    // Admins can only access if allowAdminAccess is true
    return (isAdmin && allowAdminAccess) || 
           isAdminLogin || 
           isMaintenancePage;
  };
  ...
  // Show maintenance screen for all non-admin users during maintenance
  ...
  return (
    <div>
      ...
      {/* User Pages - Only accessible when NOT in maintenance mode */}
      {!maintenanceMode && view === "dashboard" &&
      ...
      {!maintenanceMode && view === "news" && renderUserNavigation(<News user={user} setView={setView} />)}
      {!maintenanceMode && view === "calendar" && renderUserNavigation(<Calendar user={user} setView={setView} />)}
      {!maintenanceMode && view === "history" &&
        renderUserNavigation(
          <History
            user={user}
            setView={setView}
      ...
      {!maintenanceMode && view === "notification" &&
        renderUserNavigation(
          <Notification
            user={user}
            setView={setView}
      ...
      {!maintenanceMode && view === "messages" && renderUserNavigation(<Messages user={user} setView={setView} />)}
      {!maintenanceMode && view === "profile" && renderUserNavigation(<Profile user={user} setView={setView} />)}
      {!maintenanceMode && view === "editProfile" && renderUserNavigation(<EditProfile user={user} setView={setView} />)}
      {!maintenanceMode && view === "guidelines" && renderUserNavigation(<Guidelines user={user} setView={setView} />)}
      {!maintenanceMode && view === "help" && renderUserNavigation(<HelpCenter user={user} setView={setView} />)}
      {!maintenanceMode && view === "reserve" &&
        renderUserNavigation(
          <ReserveRoom
            user={user}
            setView={setView}
      ...
      {!maintenanceMode && view === "reservationDetails" && 
        renderUserNavigation(
          <ReservationDetails
      ...
      {/* Admin Pages - Only accessible when admin access is allowed during maintenance */}
      {(allowAdminAccess || !maintenanceMode) && view === "adminDashboard" && renderAdminNavigation(<AdminDashboard setView={setView} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "adminReservation" && renderAdminNavigation(<AdminReservations setView={setView} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "adminRoom" && renderAdminNavigation(<AdminRooms setView={setView} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "adminUsers" && renderAdminNavigation(<AdminUsers setView={setView} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "adminMessage" && renderAdminNavigation(<AdminMessages setView={setView} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "adminReports" && renderAdminNavigation(<AdminReports setView={setView} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "adminNotifications" && renderAdminNavigation(<AdminNotification setView={setView} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "archivedUsers" && renderAdminNavigation(<ArchivedUsers setView={setView} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "archivedReservations" && renderAdminNavigation(<ArchivedReservations setView={setView} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "archivedReports" && renderAdminNavigation(<ArchivedReports setView={setView} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "archivedNews" && renderAdminNavigation(<ArchivedNews setView={setView} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "adminNews" && renderAdminNavigation(<AdminNews setView={setView} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "adminLogs" && renderAdminNavigation(<AdminLogs setView={setView} />)}

      {/* Admin Settings Pages */}
      {(allowAdminAccess || !maintenanceMode) && view === "profileSettings" && renderAdminNavigation(<ProfileSettings setView={setView} admin={user} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "passwordSecurity" && renderAdminNavigation(<PasswordSecurity setView={setView} admin={user} />)}
      {(allowAdminAccess || !maintenanceMode) && view === "systemSettings" && renderAdminNavigation(<SystemSettings setView={setView} admin={user} />)}

      {/* Staff Pages - Only accessible when NOT in maintenance mode */}
      {!maintenanceMode && view === "staffDashboard" && renderStaffNavigation(<StaffDashboard setView={setView} staff={user} />)}
      {!maintenanceMode && view === "staffReservation" && renderStaffNavigation(<StaffReservations setView={setView} staff={user} />)}
      {!maintenanceMode && view === "staffUsers" && renderStaffNavigation(<StaffUsers setView={setView} staff={user} />)}
      {!maintenanceMode && view === "staffMessages" && renderStaffNavigation(<StaffMessages setView={setView} staff={user} />)}
      {!maintenanceMode && view === "staffNotification" && renderStaffNavigation(<StaffNotification setView={setView} staff={user} />)}
      {!maintenanceMode && view === "staffProfile" && renderStaffNavigation(<StaffProfile setView={setView} staff={user} />)}
      {!maintenanceMode && view === "staffReports" && renderStaffNavigation(<StaffReports setView={setView} staff={user} />)}
