...
useEffect(() => {
  if (maintenanceMode && isMaintenanceChecked) {
    // If user is trying to access admin login, allow it
    if (view === 'adminLogin') {
      return; // Allow access to admin login
    }
    
    // If current view is not allowed during maintenance, redirect to maintenance screen
    if (!isViewAllowed()) {
      setView("maintenance");
    }
  } else if (!maintenanceMode && view === "maintenance" && isMaintenanceChecked) {
    // Maintenance mode is off, redirect from maintenance screen
    if (user) {
      const role = user.role.toLowerCase();
      if (role === "staff") setView("staffDashboard");
      else if (role === "admin") setView("adminDashboard");
