...
useEffect(() => {
  ...
  if (path && path !== location.pathname) {
    ...
    // SPECIAL CASE: Always allow navigation to admin-login during maintenance
    if (maintenanceMode && view === 'adminLogin') {
      console.log("Allowing navigation to admin login during maintenance");
      navigate(path, { replace: false });
      return;
    }
    
    navigate(path, { replace: false }); // This line is the problem!
  }
}, [view, isInitialized]);
...
useEffect(() => {
  ...
  if (path && path !== location.pathname) {
    ...
    // CRITICAL: Always allow navigation to admin-login - NO CONDITIONS
    if (view === 'adminLogin') {
      console.log("Allowing navigation to admin login page");
      navigate(path, { replace: false });
      return;
    }
    
    // For all other routes, check maintenance restrictions
    if (maintenanceMode) {
      const isAdmin = user?.role === 'admin';
      const allowedDuringMaintenance = (isAdmin && allowAdminAccess);
      
      if (!allowedDuringMaintenance && view !== 'maintenance') {
        console.log("Blocked navigation during maintenance:", view);
        // Don't setView here to avoid loops, just don't navigate
        return;
      }
    }
...
const handleMaintenanceRedirect = (maintenanceData) => {
  ...
  // CRITICAL: Always allow admin login page - no conditions
  const canAccess = isAdminLogin || // Always allow admin login
                   (isAdmin && maintenanceData.allowAdminAccess) || 
                   isMaintenancePage;
  ...
  if (!canAccess && !isMaintenancePage) {
    ...
    console.log("Access allowed during maintenance. User:", user?.role, "View:", view);
  }
};
  useEffect(() => {
...
useEffect(() => {
  ...
  if (maintenanceMode) {
    ...
    // CRITICAL: Never redirect from adminLogin page
    if (view === 'adminLogin') {
      console.log("Keeping admin login page accessible");
      return;
    }
    ...
    // For admin users, check if they're allowed access
    if (user && user.role === 'admin' && !allowAdminAccess && view !== 'maintenance') {
      setView("maintenance");
      return;
    }
    
    // For non-logged in users, show maintenance screen if not already there
    if (!user && view !== 'maintenance' && view !== 'adminLogin') {
      setView("maintenance");
      return;
    }
  }
...
const isViewAllowed = () => {
  ...
  // During maintenance mode, allow:
  // - adminLogin page (for everyone to attempt admin login)
  // - maintenance page (the maintenance screen itself)
  // - admin pages (only if user is admin AND admin access is allowed)
  return isAdminLogin || 
         isMaintenancePage || 
  ...
  return (
    <div>
      ...
      {/* Admin Login - Always accessible even during maintenance */}
{/* Admin Login - Always accessible to everyone during maintenance */}
{view === "adminLogin" && (
  <Login_Admin
