...
const handleMaintenanceRedirect = (maintenanceData) => {
  ...
  // CRITICAL: Never redirect from admin login page
  if (view === 'adminLogin') {
    return;
  }
  ...
  const canAccess = (isAdmin && maintenanceData.allowAdminAccess) || isMaintenancePage;
...
useEffect(() => {
  ...
  if (maintenanceMode) {
    ...
    // CRITICAL: Admin users are NEVER affected by maintenance mode
    if (user?.role === 'admin') {
      console.log("Admin user detected - bypassing maintenance mode");
      return; // Admin can access everything normally
    }
    
    // CRITICAL: Always allow admin login page for everyone
    if (view === 'adminLogin') {
      console.log("Allowing admin login page for everyone");
      return;
    }
    
    // For non-admin users (regular users, staff, or logged out), show maintenance
    if (view !== 'maintenance') {
      console.log("Redirecting non-admin user to maintenance screen");
      setView("maintenance");
    }
  }
