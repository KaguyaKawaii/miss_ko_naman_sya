...
const handleMaintenanceRedirect = (maintenanceData) => {
  if (!maintenanceData.maintenanceMode) {
    // Maintenance off - redirect from maintenance screen
    if (view === "maintenance") {
      if (user) {
        const role = user.role.toLowerCase();
        if (role === "staff") setView("staffDashboard");
  ...
  // ADMIN USERS: Completely bypass maintenance mode
  if (user?.role === 'admin') {
    console.log("Admin user - bypassing maintenance mode");
    return;
  }

  // Everyone else (non-admin) gets maintenance screen
  if (view !== 'maintenance' && view !== 'adminLogin') {
    console.log("Redirecting non-admin user to maintenance");
    setView("maintenance");
  }
};
