...
useEffect(() => {
  ...
  if (path && path !== location.pathname) {
    ...
    // CRITICAL: Always allow admin login and admin users
    if (view === 'adminLogin' || user?.role === 'admin') {
      console.log("Allowing navigation for admin");
      navigate(path, { replace: false });
      return;
    }
    
    // Block navigation for non-admin users during maintenance
    if (maintenanceMode && view !== 'maintenance') {
      console.log("Blocking navigation during maintenance");
      return;
    }
    
    navigate(path, { replace: false });
  }
}, [view, isInitialized, maintenanceMode, user]);
...
const handleMaintenanceRedirect = (maintenanceData) => {
  if (!maintenanceData.maintenanceMode) {
    return; // Let the main effect handle maintenance mode off
  }
...
useEffect(() => {
  ...
  if (maintenanceMode) {
    ...
    // For non-admin users, redirect to maintenance if not already there
    if (view !== 'maintenance') {
      console.log("Redirecting non-admin user to maintenance screen");
      setView("maintenance");
    }
  } else if (view === "maintenance") {
    // Maintenance mode turned off - redirect appropriately
    if (user) {
      const role = user.role.toLowerCase();
      if (role === "staff") setView("staffDashboard");
      else if (role === "admin") setView("adminDashboard");
