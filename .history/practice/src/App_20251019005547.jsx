...
const handleMaintenanceRedirect = (maintenanceData) => {
  ...
  // CRITICAL: Never redirect from adminLogin or home pages during maintenance
  if (view === 'adminLogin' || view === 'home') {
    console.log("Keeping admin login and home pages accessible during maintenance");
    return;
  }
  ...
  const isPublicAuthPage = ['login', 'signup', 'resetPassword', 'adminLogin', 'home'].includes(view);
...
useEffect(() => {
  if (maintenanceMode && isMaintenanceChecked) {
    ...
    // Also allow access to home page so users can reach the maintenance screen
    if (view === 'home') {
      console.log("Keeping home page accessible");
      return;
    }
