...
useEffect(() => {
  if (maintenanceMode && isMaintenanceChecked) {
    ...
    // CRITICAL: Always allow access to adminLogin page during maintenance
    if (view === 'adminLogin') {
      console.log("Allowing access to admin login page during maintenance");
      return;
    }
    
    // If user is not on adminLogin and not allowed, redirect to maintenance
    const isAdmin = user?.role === 'admin';
    const isStaff = user?.role === 'staff';
