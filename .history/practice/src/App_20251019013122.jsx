...
const handleMaintenanceRedirect = (maintenanceData) => {
  ...
  // CRITICAL: Always allow adminLogin, home, and login pages - anyone can access these
  const allowedPublicRoutes = ['adminLogin', 'home', 'login'];
  if (allowedPublicRoutes.includes(view)) {
    console.log("Allowing access to public routes during maintenance");
    return;
  }

  const isAdmin = user?.role === 'admin';
  const isStaff = user?.role === 'staff';
  const isAllowedUser = (isAdmin && maintenanceData.allowAdminAccess) || isStaff;
...
useEffect(() => {
  if (maintenanceMode && isMaintenanceChecked) {
    ...
    // CRITICAL: Always allow access to adminLogin, home, and login pages during maintenance
    // Anyone should be able to access these pages from maintenance screen
    const allowedPublicRoutes = ['adminLogin', 'home', 'login', 'maintenance'];
    
    // If user is trying to access a public route, allow it
    if (allowedPublicRoutes.includes(view)) {
      console.log("Allowing access to public route during maintenance:", view);
      return;
    }
    ...
    // Only redirect to maintenance if user is not allowed and not already on maintenance page
    if (!isAllowedUser && view !== 'maintenance') {
      console.log("Redirecting to maintenance mode");
      setView("maintenance");
    }
...
const isViewAllowed = () => {
  ...
  // Always allow public routes during maintenance - ANYONE can access these
  ...
  return (isAdmin && allowAdminAccess) || isStaff;
};
...
if (maintenanceMode && isMaintenanceChecked && !isViewAllowed()) {
  ...
  return (
    <MaintenanceScreen 
      ...
      onAdminAccess={handleAdminAccessFromMaintenance} // Pass the handler
    />
  );
}
  ...
  return (
     <div>
    {/* Public Pages - These won't show during maintenance for regular users */}
    {view === "home" && !maintenanceMode && (
      ...
      {view === "maintenance" && (
        <MaintenanceScreen 
          message={maintenanceMessage}
          onAdminAccess={handleAdminAccessFromMaintenance} // Pass the handler here too
        />
      )}
