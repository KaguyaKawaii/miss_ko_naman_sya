...
function App() {
  ...
  const checkMaintenanceMode = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        console.log("Fetched maintenance status:", maintenanceData);
        
        setMaintenanceMode(maintenanceData.maintenanceMode || false);
        setMaintenanceMessage(maintenanceData.maintenanceMessage || "");
        setAllowAdminAccess(maintenanceData.allowAdminAccess !== undefined ? maintenanceData.allowAdminAccess : true);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isMaintenanceChecked) return;
    ...
    // If maintenance is OFF and we're on maintenance screen, redirect to appropriate page
    if (!maintenanceMode && view === "maintenance") {
    ...
    // If maintenance is ON, check if current view is allowed
    if (maintenanceMode) {
      const isAdmin = user?.role === 'admin';
      const isStaff = user?.role === 'staff';
      const isAllowedUser = (isAdmin && allowAdminAccess) || isStaff;
      
      // Always allow these public routes during maintenance
      const allowedPublicRoutes = ['adminLogin', 'home', 'login', 'maintenance', 'signup'];
      
      if (allowedPublicRoutes.includes(view)) {
        console.log("Allowing public route during maintenance:", view);
        return;
      }
      
      // If user is not allowed and not on maintenance page, redirect to maintenance
      if (!isAllowedUser && view !== 'maintenance') {
        console.log("Redirecting to maintenance mode");
        setView("maintenance");
      }
    }
  }, [view, user, maintenanceMode, isMaintenanceChecked, allowAdminAccess]);
  ...
  const handleAdminAccessFromMaintenance = () => {
    console.log("Admin access requested from maintenance screen");
    setView("adminLogin");
    setViewHistory(prev => [...prev, "adminLogin"]);
  };
  ...
  return (
    <div>
      {/* HEADER - Show when on home page OR during maintenance */}
      {!maintenanceMode && view === "home" && (
        <Header 
      ...
      {/* Show maintenance screen if maintenance is active and user is not allowed */}
      {maintenanceMode && isMaintenanceChecked && (
        (() => {
          const isAdmin = user?.role === 'admin';
          const isStaff = user?.role === 'staff';
          const isAllowedUser = (isAdmin && allowAdminAccess) || isStaff;
          
          if (!isAllowedUser && view !== 'adminLogin') {
      ...
      {/* Only show homepage content if NOT in maintenance mode OR user is allowed */}
      {view === "home" && (
        (() => {
          const isAdmin = user?.role === 'admin';
          const isStaff = user?.role === 'staff';
          ...
          // Show homepage content only if maintenance is OFF OR user is allowed during maintenance
          if (!maintenanceMode || (maintenanceMode && isAllowedUser)) {
            return (
              <>
      ...
      {/* User Pages */}
      {view === "dashboard" && renderUserNavigation(<Dashboard user={user} setView={setView} setSelectedReservation={setSelectedReservation} />)}
      ...
      {view === "history" && renderUserNavigation(<History user={user} setView={setView} setSelectedReservation={setSelectedReservation} refreshKey={historyRefreshKey} />)}
      {view === "notification" && renderUserNavigation(<Notification user={user} setView={setView} setSelectedReservation={setSelectedReservation} />)}
      ...
      {view === "reserve" && renderUserNavigation(<ReserveRoom user={user} setView={setView} onReservationSubmitted={() => setHistoryRefreshKey((prev) => prev + 1)} />)}
      {view === "reservationDetails" && renderUserNavigation(<ReservationDetails reservation={selectedReservation} setView={setView} refreshReservations={() => setHistoryRefreshKey((prev) => prev + 1)} user={user} />)}
