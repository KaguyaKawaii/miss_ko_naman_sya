...
function Body3() {
  ...
  useEffect(() => {
    // Scroll rooms container to top when floor changes
  ...
  return (
    <section className="relative min-h-screen bg-gradient-to-b from-gray-50 to-white text-gray-800 py-16 sm:py-24 px-4 sm:px-6 overflow-hidden">
            ...
            >
              <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
                <div className="text-center text-white p-6 sm:p-8">
                  ...
                  <p className="text-sm sm:text-base opacity-80 mt-2 max-w-md">
              ...
              {/* Control Bar */}
              <div className="absolute top-4 right-4 flex items-center space-x-2 bg-black/40 backdrop-blur-sm rounded-full px-3 py-2 z-20">
                <button
                  onClick={togglePlay}
          ...
          <div className="bg-white rounded-3xl shadow-xl p-6 sm:p-8 border border-gray-100">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                {floors[currentFloor].rooms.length} rooms
              </div>
            </div>
            
            <div 
              ref={roomsContainerRef}
              className="h-64 sm:h-72 lg:h-80 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 hover:scrollbar-thumb-gray-400"
            >
              <div className="space-y-3 pr-2">
                {floors[currentFloor].rooms.map((room, index) => (
                  <button
                    ...
                    className="w-full text-left p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-all duration-300 border border-gray-200 hover:border-gray-300 hover:shadow-md group"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <div className={`w-2 h-2 rounded-full ${
                            floors[currentFloor].color.split(' ')[0].replace('bg-gradient-to-br', 'bg')
                          }`}></div>
      ...
      {selectedRoom && (
        ...
        >
          <div 
            className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 sm:p-8 animate-scale-in"
          ...
          >
            ...
            <div className={`${floors[currentFloor].color} h-48 rounded-2xl mb-6 flex items-center justify-center`}>
              <span className="text-white text-4xl bg-black/20 rounded-full p-4">
                {floors[currentFloor].icon}
              </span>
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-6">
              <div className="text-sm text-gray-600">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span>{floors[currentFloor].level}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="font-semibold">Floor Type:</span>
                  <span>{floors[currentFloor].title}</span>
                </div>
              </div>
            </div>
