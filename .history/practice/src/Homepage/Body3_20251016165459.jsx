...
// Import additional images for 4th and 5th floors
...
const floors = [
  ...
  },
  {
    ...
    icon: "🏢",
    rooms: [
      { src: Picture2, label: "Collaboration Room", description: "High-tech environment for collaborative learning and team projects" },
...
function Body3() {
  ...
  return (
    <section className="relative min-h-screen bg-gradient-to-b from-gray-50 to-white text-gray-800 py-12 sm:py-16 lg:py-24 px-4 sm:px-6 lg:px-8 overflow-hidden flex flex-col items-center">
      ...
      {/* Floor Navigation */}
      <div className="w-full max-w-4xl mx-auto mb-8 sm:mb-12 lg:mb-16">
        <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-4 lg:space-x-6">
          {floors.map((floor, floorIndex) => (
            <button
              ...
              className={`px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-semibold text-sm sm:text-base lg:text-lg transition-all duration-300 transform hover:scale-105 hover:shadow-lg flex items-center space-x-3 min-w-[140px] sm:min-w-[160px] ${
            ...
            {floors[activeFloorIndex].rooms.map((room, roomIndex) => {
              ...
              return (
                <button
                  ...
                  className={`group relative h-64 sm:h-72 lg:h-80 rounded-xl sm:rounded-2xl overflow-hidden transition-all duration-500 ${
                    globalIndex === index 
                ...
                >
                  ...
                  <div className={`absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end justify-start p-4 sm:p-6 transition-all duration-300 ${
                    ...
                    <div className="text-left">
                      <h4 className="text-white font-semibold text-lg sm:text-xl lg:text-2xl mb-2 drop-shadow-md">
                        {room.label}
                      </h4>
        ...
        <div className="mt-12 sm:mt-16 lg:mt-20 bg-gradient-to-r from-gray-50 to-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-200 p-6 sm:p-8 text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8 max-w-4xl mx-auto">
            {floors.map((floor, index) => (
              <div key={floor.name} className="bg-white rounded-xl p-6 shadow-lg border border-gray-100">
                <div className={`w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-r ${floor.color} flex items-center justify-center text-white text-xl font-bold`}>
                  {index + 1}
                </div>
                <h4 className="font-bold text-gray-900 text-lg sm:text-xl mb-2">{floor.name}</h4>
                <p className="text-gray-600 text-sm sm:text-base mb-3">{floor.description}</p>
                <div className="text-2xl sm:text-3xl font-bold text-[#CC0000]">{floor.rooms.length} rooms</div>
              </div>
            ))}
          </div>
          <div className="mt-8 pt-8 border-t border-gray-200">
            <p className="text-gray-600 text-lg sm:text-xl">
              Total of <span className="font-bold text-[#CC0000] text-xl sm:text-2xl">{allSlides.length} rooms</span> available across all floors
            </p>
          </div>
        </div>
      </div>
