...
function StaffDashboard({ staff, setView }) {
  ...
  // Fetch reservations for the staff's floor
  const fetchReservations = async () => {
    ...
    try {
      setLoading(true);
      // Use the correct endpoint for reservations by floor
      ...
      // Fallback to fetching all reservations and filtering
      try {
  ...
  // Fetch messages for the staff
  const fetchMessages = async () => {
    try {
      // Get staff recipients to see message counts
      ...
      setMessages(recipients);
      return { recipients, unreadCount };
    } catch (err) {
      console.error("Error fetching messages:", err);
      setMessages([]);
      return { recipients: [], unreadCount: 0 };
    }
  };
  ...
  const fetchUniqueUsersCount = async (reservationsData) => {
    try {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [reservationsData, messagesData, notificationsData] = await Promise.all([
        fetchReservations(),
        fetchMessages(),
        fetchNotifications()
      ]);
      ...
      const uniqueUsersCount = await fetchUniqueUsersCount(reservationsData);
      ...
      const messageActivities = (messagesData.recipients || []).slice(0, 2).map((msg, index) => ({
