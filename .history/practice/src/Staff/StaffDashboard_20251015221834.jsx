...
function StaffDashboard({ staff, setView }) {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const endpoints = [
        { key: 'reservations', url: `/reservations/floor?floor=${encodeURIComponent(staff.floor)}` },
        { key: 'messages', url: `/messages/staff-recipients/${staff._id}` },
        { key: 'notifications', url: `/notifications/staff/${staff._id}?filter=unread` },
        { key: 'news', url: '/news' }
      ];
  ...
  const processFetchedData = (data) => {
    ...
    // Filter reservations by staff's floor
    ...
    setReservations(filteredReservations.sort((a, b) => {
    ...
    updateRecentActivity(filteredReservations, messagesData);
  };
  ...
  const updateRecentActivity = (reservationsData, messagesData) => {
    ...
    const messageActivities = Array.isArray(messagesData) ? 
      messagesData.slice(0, 2).map((msg, index) => ({
