...
function StaffDashboard({ staff, setView }) {
  ...
  const processFetchedData = (data) => {
    ...
    const totalUsers = safeLength(allUsers);

    // Process messages - get all messages for staff
    const allMessages = data.messages?.error ? [] : data.messages;
    const staffMessages = Array.isArray(allMessages) ? 
      allMessages.filter(message => 
        message.recipientId === staff._id || message.recipientType === 'staff'
      ) : [];
    const unreadMessagesCount = staffMessages.filter(msg => !msg.read).length;

    // Process notifications - get all notifications for staff
    const allNotifications = data.notifications?.error ? [] : data.notifications;
    const staffNotifications = Array.isArray(allNotifications) ? 
      allNotifications.filter(notification => 
        notification.recipientId === staff._id || notification.recipientType === 'staff'
      ) : [];
    const unreadNotificationsCount = staffNotifications.filter(notif => !notif.read).length;
    ...
    updateRecentActivity(sortedReservations, staffMessages, staffNotifications);
  };

  const updateRecentActivity = (reservationsData, messagesData, notificationsData) => {
    ...
    const messageActivities = (Array.isArray(messagesData) ? messagesData.slice(0, 2) : []).map((msg, index) => ({
      id: msg._id || `msg-${index}`,
      action: `New message from ${msg.senderName || msg.name || 'User'}`,
      time: msg.createdAt ? new Date(msg.createdAt).toLocaleDateString() : "Recently",
      user: msg.senderName || msg.name || 'User',
      type: "message",
    }));

    const notificationActivities = (Array.isArray(notificationsData) ? notificationsData.slice(0, 2) : []).map((notif, index) => ({
      id: notif._id || `notif-${index}`,
      action: `Notification: ${notif.title || 'New notification'}`,
      time: notif.createdAt ? new Date(notif.createdAt).toLocaleDateString() : "Recently",
      user: "System",
      type: "notification",
    }));

    setRecentActivity([...reservationActivities, ...messageActivities, ...notificationActivities].slice(0, 4));
  };
