...
function StaffDashboard({ staff, setView }) {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const endpoints = [
        ...
        { key: 'messages', url: `/messages/staff-recipients/${staff._id}` },
        { key: 'notifications', url: `/notifications/staff/${staff._id}` },
        { key: 'users', url: '/users' },
        { key: 'news', url: '/news' }
      ];
  ...
  const processFetchedData = (data) => {
    ...
    // Process messages - use staff recipients data
    const messagesData = data.messages?.error ? [] : data.messages;
    const unreadMessagesCount = Array.isArray(messagesData) ? 
      messagesData.reduce((total, recipient) => total + (recipient.unreadCount || 0), 0) : 0;

    // Process notifications - use staff notifications data
    const notificationsData = data.notifications?.error ? [] : data.notifications;
    const unreadNotificationsCount = Array.isArray(notificationsData) ? 
      notificationsData.filter(notif => !notif.isRead).length : 0;
    ...
    updateRecentActivity(sortedReservations, messagesData, notificationsData);
  };
  ...
  const updateRecentActivity = (reservationsData, messagesData, notificationsData) => {
    ...
    const notificationActivities = (Array.isArray(notificationsData) ? notificationsData.slice(0, 2) : []).map((notif, index) => ({
      ...
      action: `Notification: ${notif.message || 'New notification'}`,
