...
function StaffDashboard({ staff, setView, unreadCounts, onRefreshCounts }) {
  ...
  // Enhanced data fetching with fallbacks for missing endpoints
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      // Try different reservation endpoints - start with the most common ones
      ...
      // Try each reservation endpoint until one works
      for (const endpoint of reservationEndpoints) {
        try {
          const response = await apiService.get(endpoint);
          reservationsData = Array.isArray(response) ? response : (response.reservations || response.bookings || []);
          console.log(`Found reservations at ${endpoint}:`, reservationsData.length);
      ...
      // If no reservation endpoint worked, use empty array
      if (reservationsData.length === 0) {
        console.log("No reservation endpoints available, using empty data");
        reservationsData = [];
      }

      // Fetch other data with fallbacks
      const endpoints = [
        { key: 'messages', url: `/messages/staff-total-unread/${staff._id}`, fallback: { count: 0 } },
        { key: 'notifications', url: `/notifications/unread-count/${staff._id}`, fallback: { count: 0 } },
        { key: 'users', url: '/users', fallback: { users: [] } },
        { key: 'news', url: '/news', fallback: [] }
      ];
  ...
  const processFetchedData = (data) => {
    ...
    const filteredReservations = Array.isArray(allReservations) ? 
      allReservations.filter(reservation => {
        const reservationFloor = reservation.location || reservation.floor || reservation.roomLocation || "";
    ...
    const sortedReservations = Array.isArray(filteredReservations) ? 
      filteredReservations.sort((a, b) => {
        const aTime = a.createdAt ? new Date(a.createdAt) : new Date(a.date || a.datetime || 0);
        const bTime = b.createdAt ? new Date(b.createdAt) : new Date(b.date || b.datetime || 0);
        return bTime - aTime;
      }) : [];
  ...
  const updateRecentActivity = (reservationsData) => {
    const reservationActivities = (Array.isArray(reservationsData) ? reservationsData.slice(0, 3) : []).map((r) => ({
      ...
      action: `Reservation in ${r.roomName || r.room || r.location || 'Unknown Room'}`,
      time: r.datetime ? new Date(r.datetime).toLocaleDateString() : 
            r.date ? new Date(r.date).toLocaleDateString() : "Unknown date",
      user: r.userId?.name || r.userName || "Unknown",
      type: "reservation",
      status: r.status || 'Unknown'
    }));

    // Add demo activities if no real data
    if (reservationActivities.length === 0) {
      const demoActivities = [
        {
          id: 'demo-1',
          action: 'New reservation request',
      ...
      setRecentActivity(demoActivities);
    } else {
  ...
  const handlePostNews = () => {
    ...
    .catch((err) => {
      ...
      alert("Failed to post news. The news endpoint may not be available.");
    });
  };
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
              ...
              {reservations.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-400 text-sm mt-2">Reservation data will appear here when available.</p>
                </div>
              ) : (
                    ...
                    >
                      <div className="flex justify-between items-start mb-1">
                        <h3 className="font-medium text-gray-800">
                          {r.roomName || r.room || r.location || 'Unknown Room'}
                        </h3>
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                          (r.status === 'Pending' || r.status === 'pending') ? 'bg-yellow-100 text-yellow-800' :
                          (r.status === 'Approved' || r.status === 'approved') ? 'bg-green-100 text-green-800' :
                          (r.status === 'Ongoing' || r.status === 'ongoing') ? 'bg-blue-100 text-blue-800' :
                      ...
                      <p className="text-sm text-gray-600">
                        Reserved by {r.userId?.name || r.userName || "Unknown"}
                      </p>
