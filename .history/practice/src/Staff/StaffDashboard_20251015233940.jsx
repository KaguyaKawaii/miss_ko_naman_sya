...
function StaffDashboard({ staff, setView, unreadCounts, onRefreshCounts }) {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const endpoints = [
        ...
        { key: 'messages', url: `/messages/staff-total-unread/${staff._id}` }, // Corrected endpoint
        { key: 'notifications', url: `/notifications/unread-count/${staff._id}` }, // Corrected endpoint
        { key: 'users', url: '/users' },
        { key: 'news', url: '/news' }
      ];
  ...
  // Floor reservation data for display
  const floorStats = [
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <p className="text-sm text-gray-500">
              Total across all floors: {Object.values(floorReservations).reduce((sum, count) => sum + count, 0)}
            </p>
          </div>
              ...
              <div key={floor.name} className="bg-gradient-to-br from-gray-50 to-gray-100 p-4 rounded-lg border border-gray-200 hover:shadow-md transition-all">
                ...
                <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${Math.min((floor.count / Math.max(...floorStats.map(f => f.count)) * 100) || 0, 100)}%` }}
                  ></div>
                </div>
              </div>
            ))}
