  ...
  const processFetchedData = (data) => {
    ...
    filteredReservations.forEach(reservation => {
      ...
      if (reservation.userId?.id) {
        uniqueUserIds.add(reservation.userId.id.toString());
      }
      if (reservation.participants && Array.isArray(reservation.participants)) {
        reservation.participants.forEach(participant => {
          if (participant.id_number || participant._id) {
            uniqueUserIds.add(participant.id_number || participant._id.toString());
          }
        });
      }
    });
    ...
    const unreadMessagesCount = Array.isArray(messagesData) ? 
      messagesData.reduce((total, conversation) => total + (conversation.unreadCount || 0), 0) : 0;
    ...
    setSummaryData({
      ...
      notifications: safeLength(notificationsData)
    });
    ...
    updateRecentActivity(sortedReservations, messagesData);
  };

  const updateRecentActivity = (reservationsData, messagesData) => {
    ...
    const messageActivities = (Array.isArray(messagesData) ? messagesData.slice(0, 2) : []).map((msg, index) => ({
      ...
      action: `New message from ${msg.name || msg.senderName || 'User'}`,
      time: msg.latestMessageTimestamp ? new Date(msg.latestMessageTimestamp).toLocaleDateString() : 
            msg.createdAt ? new Date(msg.createdAt).toLocaleDateString() : "Recently",
      user: msg.name || msg.senderName || 'User',
      type: "message",
    }));
