...
function StaffDashboard({ staff, setView, unreadCounts, onRefreshCounts }) {
  ...
  const fetchFloorReservations = useCallback(async () => {
    try {
      ...
      for (const floor of floors) {
        try {
          const response = await apiService.get(`/api/reservations/floor/${encodeURIComponent(floor)}`);
          floorData[floor] = Array.isArray(response) ? response.length : 0;
        } catch (error) {
          console.log(`No reservations found for ${floor} or endpoint not available`);
          floorData[floor] = 0;
        }
      }
  ...
  // Enhanced data fetching with CORRECT endpoints
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const endpoints = [
        ...
        { key: 'messages', url: `/api/messages/staff-total-unread/${staff._id}` },
        { key: 'notifications', url: `/api/notifications/unread-count/${staff._id}` },
  ...
  }, [staff, onRefreshCounts, fetchFloorReservations]);
  ...
  const processFetchedData = (data) => {
    ...
    const allReservations = data.reservations?.error ? [] : data.reservations;
    ...
    const allUsers = data.users?.error ? [] : data.users;
    const totalUsers = safeLength(allUsers.users || allUsers);

    // Process messages - use the count directly from the API
    const messagesData = data.messages?.error ? { count: 0 } : data.messages;
    const unreadMessagesCount = messagesData.count || 0;

    // Process notifications - use the count directly from the API
    const notificationsData = data.notifications?.error ? { count: 0 } : data.notifications;
  ...
  return (
    <main className="ml-[250px] w-[calc(100%-250px)] min-h-screen bg-gray-50">
      ...
      <div className="p-8">
        ...
        {/* FLOOR RESERVATIONS OVERVIEW SECTION */}
