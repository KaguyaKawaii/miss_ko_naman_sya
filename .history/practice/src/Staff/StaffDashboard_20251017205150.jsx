...
function StaffDashboard({ staff, setView, unreadCounts, onRefreshCounts }) {
    ...
    try {
      ...
      const fetchPromises = endpoints.map(endpoint => 
        apiService.get(endpoint.url).catch(error => {
          ...
          if (endpoint.key === 'users') return { users: [] };
          return { count: 0 };
        })
      );
  ...
  const processFetchedData = (data) => {
    ...
    // Process users - get all users and filter out staff
    const allUsers = data.users || {};
    const usersArray = Array.isArray(allUsers.users) ? allUsers.users : 
                      Array.isArray(allUsers) ? allUsers : [];
    
    // Filter out staff users - only count regular users
    ...
    console.log('👥 User statistics:', {
      ...
      staffUsers: usersArray.length - regularUsers.length
    });
    ...
    setSummaryData({
      ...
      users: totalRegularUsers, // Now only counts regular users, not staff
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300 transform hover:-translate-y-1">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500 text-sm font-medium mb-2">Regular Users</p>
                <p className="text-3xl font-bold text-gray-800 mb-2">
                  {summaryData.users}
                </p>
                <div className="flex items-center gap-1 text-xs text-gray-600">
                  <Users size={12} />
                  <span>Excluding staff accounts</span>
                </div>
              </div>
