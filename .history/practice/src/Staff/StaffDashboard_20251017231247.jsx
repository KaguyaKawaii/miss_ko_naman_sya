...
// API service module - FIXED baseURL
const apiService = {
  baseURL: "http://localhost:5000/api", // Keep /api here since your backend uses /api routes
...
function StaffDashboard({ staff, setView, unreadCounts, onRefreshCounts }) {
  ...
  // Test each endpoint individually to see which ones work
  const testEndpoints = async () => {
    ...
    const testUrls = [
      ...
      '/users', 
      '/rooms',
      `/messages/staff-total-unread/${staff._id}`,
      `/notifications/unread-count/${staff._id}`
    ];
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      // FIRST: Test which endpoints are working
      await testEndpoints();
      
// In StaffDashboard.jsx - update the endpoints array:
const endpoints = [
  { key: 'reservations', url: `/reservations` }, // This becomes /api/reservations/reservations
  { key: 'messages', url: `/messages/staff-total-unread/${staff._id}` },
  { key: 'notifications', url: `/notifications/unread-count/${staff._id}` },
  { key: 'users', url: '/reservations/users' }, // Now using the new route
  { key: 'rooms', url: '/reservations/rooms' } // Now using the new route
];
  ...
  const processFetchedData = (data) => {
    ...
    // Process users - handle different response formats
    let allUsers = [];
    ...
    if (typeof data.messages === 'number') {
      ...
      // Try to extract count from message object
    ...
    if (typeof data.notifications === 'number') {
      ...
      // Try to extract count from notification object
