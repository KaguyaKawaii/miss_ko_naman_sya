...
// Replace Room icon with a different one or use a simple div
const RoomIcon = () => (
...
const endpoints = [
  { key: 'reservations', url: `/reservations?userId=${staff._id}` },
      ...
      const fetchPromises = endpoints.map(endpoint => 
        apiService.get(endpoint.url).catch(error => {
          ...
          if (endpoint.key === 'rooms') return []; // Return empty array for rooms
          if (endpoint.key === 'messages') return 0;
          if (endpoint.key === 'notifications') return 0;
          return null;
        })
      );
  ...
  const processFetchedData = (data) => {
    ...
    // Process reservations - filter by staff's floor (EXACTLY LIKE StaffReservations.jsx)
    ...
    // FILTER RESERVATIONS BY STAFF'S FLOOR - JUST LIKE IN StaffReservations.jsx
    ...
    console.log('📈 Filtered reservations for dashboard:', filteredReservations.length);
    ...
    // Update summary data - USE FILTERED RESERVATIONS COUNT
    setSummaryData({
      reservations: safeLength(filteredReservations), // This now shows only floor-specific count
      users: totalRegularUsers,
      pendingReservations: filterByStatus(filteredReservations, 'Pending'), // This now shows only floor-specific pending
      messages: finalMessagesCount,
      notifications: finalNotificationsCount
    });

    // Set reservations data - USE FILTERED RESERVATIONS
