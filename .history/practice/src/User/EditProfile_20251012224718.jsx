...
const onImageLoaded = useCallback((img) => {
  ...
  // Fix NaN error by ensuring valid dimensions
  const containerWidth = img.naturalWidth || img.width || 100;
  const containerHeight = img.naturalHeight || img.height || 100;
  
  // Ensure we have valid numbers
  if (!containerWidth || !containerHeight || containerWidth <= 0 || containerHeight <= 0) {
    console.warn('Invalid image dimensions:', containerWidth, containerHeight);
    // Set safe defaults
    setCrop({ 
      unit: "%", 
      width: 80, 
  ...
  // Ensure cropSize is a valid number
  const safeCropSize = isNaN(cropSize) ? 80 : Math.max(10, Math.min(90, cropSize));
  
  setCrop({ 
    unit: "%", 
    width: safeCropSize, 
    height: safeCropSize, 
    x: (100 - safeCropSize) / 2, 
    y: (100 - safeCropSize) / 2,
    aspect: 1 
  });
...
const handleSaveCropped = async () => {
  ...
  try {
    ...
    // FIXED: Corrected API endpoint to match backend route
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      // FIXED: Changed the URL to match the backend route structure
  ...
  onChange={(newCrop) => {
    ...
    if (newCrop && typeof newCrop.width === 'number' && !isNaN(newCrop.width) && 
        typeof newCrop.height === 'number' && !isNaN(newCrop.height)) {
      setCrop(newCrop);
    }
  }}
