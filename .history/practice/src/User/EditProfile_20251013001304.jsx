...
// Reusable Input Component - MOVED TO TOP LEVEL
...
// ✅ FIXED: Update all API calls to use consistent endpoints
const handleProfileSubmit = async (e) => {
  ...
  try {
    ...
    setError(err.response?.data?.message || "Failed to update profile. Please check server connection.");
  } finally {
    setLoading(false);
  }
};
...
const handlePasswordSubmit = async (e) => {
  ...
  try {
    ...
    console.error("Error details:", err.response?.data);
    setError(err.response?.data?.message || "Failed to change password. Please check server connection.");
  } finally {
    setLoading(false);
  }
};
...
const handleSaveCropped = async () => {
  ...
  try {
    ...
    console.error("Error details:", err.response?.data);
    setError(err.response?.data?.message || "Failed to upload image. Please check server connection.");
  } finally {
    setUploading(false);
  }
};
...
const resetProfilePicture = async () => {
  try {
    ...
    const response = await api.delete(`/users/${user._id}/remove-picture`);
...
const fetchUserProfile = async () => {
  try {
    ...
    if (response.data.success) {
      setForm({
        ...
        year_level: response.data.user.year_level,
        floor: response.data.user.floor || ""
      });

      if (response.data.user.profilePicture) {
        const timestamp = new Date().getTime();
        const profileUrl = response.data.user.profilePicture.startsWith("http")
          ? `${response.data.user.profilePicture}?t=${timestamp}`
          : `http://localhost:5000${response.data.user.profilePicture}?t=${timestamp}`;
        setProfileUrl(profileUrl);
      }
    }
  } catch (err) {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      // ✅ FIXED: Use the correct API path with /api prefix
      await api.put(`/api/users/${user._id}/change-password`, {
        oldPassword,
        newPassword,
      });
          ...
          <div className="w-full max-w-4xl bg-white rounded-xl sm:rounded-2xl shadow-2xl border border-gray-100 overflow-hidden transform transition-all duration-300 max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-auto p-4 sm:p-6">
              <div className="space-y-4">
                {!modalImgSrc ? (
                  <div className="flex flex-col items-center justify-center py-12 sm:py-16 border-2 border-dashed border-gray-300 rounded-xl bg-gray-50 transition-all hover:border-red-400 hover:bg-red-50/30">
                    ...
                    <p className="text-gray-600 text-center mb-4 text-sm sm:text-base">Select an image to crop</p>
                    <input
                      ref={fileInputRef}
                      type="file"
                      accept="image/*"
                    ...
                    <button
                      ...
                      className="bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white py-2.5 px-6 rounded-lg font-medium transition-all shadow-sm hover:shadow-md active:scale-[0.98] cursor-pointer text-sm sm:text-base"
                    >
                      Choose Image
                    </button>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      {modalImgSrc && (
                        <ReactCrop
                          crop={crop}
                          onChange={(newCrop) => {
                            // Validate all crop values before setting state
                            if (newCrop && 
                                typeof newCrop.width === 'number' && !isNaN(newCrop.width) && newCrop.width > 0 &&
                                typeof newCrop.height === 'number' && !isNaN(newCrop.height) && newCrop.height > 0) {
                              setCrop(newCrop);
                            }
                          }}
                        ...
                        >
                          <img
                            ...
                            style={{ display: 'block' }}
                            crossOrigin="anonymous" // Add this for better CORS handling
                          />
                        </ReactCrop>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 justify-end">
                      <button
                        ...
                        className="px-4 sm:px-6 py-2.5 border border-gray-300 text-gray-700 hover:bg-gray-50 rounded-lg font-medium transition-all shadow-sm hover:shadow-md active:scale-[0.98] cursor-pointer text-sm sm:text-base"
                      >
                      ...
                      <button
                        ...
                        className="px-4 sm:px-6 py-2.5 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white rounded-lg font-medium transition-all disabled:opacity-50 flex items-center justify-center shadow-sm hover:shadow-md active:scale-[0.98] cursor-pointer text-sm sm:text-base"
                      >
                        {uploading ? (
                          <>
