...
const handleProfileSubmit = async (e) => {
  ...
  try {
    ...
    throw new Error(`All profile update endpoints returned 404. Please check server routing.`);

  } catch (err) {
    console.error("Profile update failed:", err);
    
    if (err.response?.status === 404) {
      setError(`Profile update failed: Endpoint not found (404). Please contact administrator.`);
    } else if (err.message.includes("All profile update endpoints")) {
...
// 📌 Change Password - FIXED with better debugging
  ...
  try {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        console.log(`❌ Failed with ${endpoint}:`, err.response?.data);
    ...
    if (!success) {
      throw new Error("All password change endpoints failed. Please check server configuration.");
    }
    ...
    } else if (err.message.includes("All password change endpoints")) {
      setError("Password change service is currently unavailable. Please try again later.");
    } else {
