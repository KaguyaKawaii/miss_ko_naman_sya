...
function Navigation_User({ user: initialUser, setView, currentView, onLogout }) {
  ...
  // Help dropdown ref for detecting outside clicks (desktop)
  ...
  useEffect(() => {
    ...
    // Handle clicks outside help dropdown (desktop)
  ...
  // DIFFERENT HELP BEHAVIOR FOR MOBILE VS DESKTOP
  const handleHelpClick = () => {
    ...
    // DESKTOP: Toggle right-side dropdown
    else {
      setShowHelp(prev => !prev);
    }
  };

  const handleHelpOptionClick = (viewId) => {
    setView(viewId);
    setShowHelp(false);
  ...
  return (
    <>
      ...
      {/* MOBILE: Centered Help Modal */}
      {showHelp && isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex items-center justify-center">
          <div 
            className="absolute inset-0 bg-black/50 backdrop-blur-sm"
            onClick={handleCloseHelp}
          />
          <div className="relative bg-white rounded-xl shadow-lg border border-gray-200 p-6 w-[90%] max-w-[320px] mx-4">
            <div className="flex flex-col space-y-4">
              <button
                onClick={() => handleHelpOptionClick("help")}
              ...
              <button
                onClick={() => handleHelpOptionClick("guidelines")}
                className="bg-white border border-gray-200 hover:border-gray-300 transition-all duration-200 shadow-sm rounded-xl w-full flex flex-col items-center justify-center text-center p-4 cursor-pointer focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50"
          ...
          `}>
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="flex flex-col gap-2">
                ...
                {/* Help Button - DIFFERENT BEHAVIOR FOR MOBILE VS DESKTOP */}
                <div className="relative" ref={helpDropdownRef}>
                  <button
                    onClick={handleHelpClick}
                    disabled={user?.suspended}
                    className={`
                      flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-all duration-200 justify-start cursor-pointer w-full group
                      ${showHelp || currentView === "help" || currentView === "guidelines"
                        ? "bg-red-600 text-white shadow-md"
                        : "bg-[#2a2a2a] text-gray-300 hover:bg-[#333333] hover:text-white"
                      }
                      ${user?.suspended ? 'opacity-50 cursor-not-allowed' : ''}
                  ...
                  >
                    ...
                    {/* Mobile: Show plus icon indicating modal */}
                  ...
                  {/* DESKTOP: Right-side Dropdown Menu */}
                  {showHelp && !user?.suspended && (
                    <div className="hidden lg:block absolute top-0 left-full ml-2 z-50">
                      <div className="flex flex-col bg-white rounded-xl shadow-lg border border-gray-200 p-4 w-[260px] space-y-3">
                        <button
                          onClick={() => handleHelpOptionClick("help")}
