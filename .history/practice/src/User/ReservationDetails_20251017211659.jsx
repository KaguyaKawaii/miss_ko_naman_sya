...
import { availableRoomImages, getRoomImageByUrl } from "../data/roomImages"; // Import the room images configuration
...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  const refreshReservationData = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:5000/reservations/${localReservation._id}`);
  ...
  const handleAction = async (action, data = {}) => {
    ...
    try {
      ...
      switch (action) {
        case "approve":
          endpoint = `http://localhost:5000/reservations/${localReservation._id}/status`;
          method = "patch";
        ...
        case "start":
          endpoint = `http://localhost:5000/reservations/start/${localReservation._id}`;
          method = "post";
          break;

        case "end-early":
          endpoint = `http://localhost:5000/reservations/${localReservation._id}/end-early`;
          method = "put";
          break;

        case "cancel":
          endpoint = `http://localhost:5000/reservations/${localReservation._id}`;
          method = "delete";
          break;

        case "request-extension":
          endpoint = `http://localhost:5000/reservations/${localReservation._id}/request-extension`;
          method = "put";
          requestData = { 
        ...
        case "approve-extension":
          endpoint = `http://localhost:5000/reservations/${localReservation._id}/handle-extension`;
          method = "put";
  ...
  // Function to get the appropriate floor/room image
  const getFloorImage = () => {
    if (!localReservation) return null;
    
    const { location, roomName } = localReservation;
    
    // First, try to find by exact room name match
    ...
    // If no exact room match, try to find by location/floor
    const floorImage = availableRoomImages.find(img => 
      img.name.toLowerCase().includes(location.toLowerCase()) ||
      location.toLowerCase().includes(img.name.toLowerCase())
    );
    
    if (floorImage) return floorImage;
    
    // Default to Ground Floor if no match found
    return availableRoomImages.find(img => img.id === "ground_floor");
  };
  ...
  return (
    <main className="ml-0 lg:ml-[250px] w-full lg:w-[calc(100%-250px)] flex flex-col min-h-screen bg-gray-50">
      <header className="text-black px-4 lg:px-6 h-[60px] flex items-center justify-between shadow-sm bg-white">
        <div className="flex items-center space-x-4">
          ...
          <button 
            ...
            className="text-sm text-gray-500 hover:text-gray-700 flex items-center cursor-pointer p-2 hover:bg-gray-100 rounded-lg transition-colors"
            title="Refresh data"
          >
      ...
      <div className="p-4 lg:p-6 space-y-6">
        ...
        {/* Room/Floor Image - Always show if image is available */}
        {floorImage && (
          <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100">
            <h2 className="text-lg font-semibold text-gray-800 mb-4 pb-2 border-b border-gray-100">
              {floorImage.category === "Floor" ? "Floor Location" : "Room Location"}
            </h2>
            <div className="flex flex-col justify-center items-center">
              ...
              <p className="font-semibold text-base lg:text-lg mt-4 bg-gray-100 px-4 py-2 rounded-lg border border-gray-200">
                {floorImage.name} - {localReservation.location}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-wrap justify-center gap-3 lg:gap-4 pt-4">
            ...
            <>
              ...
              {localReservation.status === "Approved" && (
                ...
                >
                  ...
                  {processingAction === "start" ? "Starting..." : "Start Session"}
                </button>
              )}
              ...
              {localReservation.status === "Ongoing" && (
                ...
                >
                  ...
                  {processingAction === "end-early" ? "Ending..." : "End Session Early"}
                </button>
              )}
            ...
            <>
              ...
              {canRequestExtension && (
                <button
                  ...
                  className="px-4 lg:px-5 py-2 lg:py-2.5 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors flex items-center cursor-pointer text-sm lg:text-base"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 lg:h-5 lg:w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
          ...
          <button
            onClick={() => setView("dashboard")}
            className="px-4 lg:px-5 py-2 lg:py-2.5 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors flex items-center cursor-pointer text-sm lg:text-base"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 lg:h-5 lg:w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M9.707 14.707a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 1.414L7.414 9H15a1 1 0 110 2H7.414l2.293 2.293a1 1 0 010 1.414z" clipRule="evenodd" />
            </svg>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            ...
            <div className="flex justify-end space-x-3">
              ...
              >
                {processingAction === "end-early" ? "Ending..." : "Yes, End Session"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            ...
            <div className="flex justify-end space-x-3">
              ...
              >
                {processingAction === "start" ? "Starting..." : "Yes, Start Session"}
              </button>
            </div>
          </div>
        </div>
      ...
      {showExtendModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            ...
            <p className="text-gray-600 mb-4">Are you sure you want to request an extension for this session?</p>
            <div className="flex justify-end space-x-3">
              <button
          ...
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            ...
            <div className="flex justify-end">
              <button
                ...
                className="px-4 py-2 bg-[#CC0000] text-white rounded-lg hover:bg-red-700 cursor-pointer"
              >
                OK
              </button>
            </div>
