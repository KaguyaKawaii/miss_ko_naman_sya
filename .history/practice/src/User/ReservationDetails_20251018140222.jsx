...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  const refreshReservationData = async () => {
    ...
    try {
      ...
      if (error.response?.status === 404) {
        ...
        // Clear the interval since reservation doesn't exist
        if (refreshInterval) {
  ...
  const handleAction = async (action, data = {}) => {
    ...
    try {
      ...
      // If it's a 404, stop auto-refresh
      if (err.response?.status === 404 && refreshInterval) {
        clearInterval(refreshInterval);
      }
    } finally {
      setProcessingAction("");
      ...
      <header className="text-black px-4 lg:px-6 h-[60px] flex items-center justify-between shadow-sm bg-white">
        <div className="flex items-center space-x-4">
          ...
          <button 
            ...
            className="text-sm text-gray-500 hover:text-gray-700 flex items-center cursor-pointer p-2 hover:bg-gray-100 rounded-lg"
            title="Refresh reservation data"
          >
