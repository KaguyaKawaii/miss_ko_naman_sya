...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  // ✅ ADDED: Participant removal states
  ...
  // ✅ ADDED: Handle participant removal
  const handleRemoveParticipant = async (participantId) => {
    ...
    try {
      const response = await axios.delete(
        ...
        {
          data: { userId: user._id } // Main reserver ID
        }
      );
  ...
  return (
    <main className="ml-0 lg:ml-[250px] w-full lg:w-[calc(100%-250px)] flex flex-col min-h-screen bg-gray-50">
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-[#FFCC00]">
                  <tr>
                    ...
                    {isMainReserver && localReservation.status === "Pending" && (
                      <th className="px-3 py-2 lg:px-4 lg:py-3 text-left text-xs font-medium text-gray-800 uppercase tracking-wider">Action</th>
                    )}
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {localReservation.participants.map((p, i) => (
                    <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50 hover:bg-gray-100"}>
                      ...
                      {/* ✅ ADDED: Remove button for each participant */}
                      {isMainReserver && localReservation.status === "Pending" && (
                        <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap">
                          <button
                            onClick={() => {
                              setRemovingParticipant(p.id_number || p.idNumber);
                              setShowRemoveConfirm(true);
                            }}
                            disabled={removingParticipant === (p.id_number || p.idNumber)}
                            className="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700 disabled:opacity-50 transition-colors text-xs cursor-pointer"
                            title="Remove participant"
                          >
                            {removingParticipant === (p.id_number || p.idNumber) ? "Removing..." : "Remove"}
                          </button>
                        </td>
                      )}
                    </tr>
      ...
      {/* ✅ ADDED: Remove Participant Confirmation Modal */}
      {showRemoveConfirm && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-200">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to remove this participant? They will be notified and will lose access to this reservation.
            </p>
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={() => handleRemoveParticipant(removingParticipant)}
