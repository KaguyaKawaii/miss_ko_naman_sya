              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {localReservation.participants.map((p, i) => {
                    ...
                    const isMainReserverUser = isMainReserver && participantId === user?.id_number;
                    const canRemoveSelf = isCurrentUser && !isMainReserverUser;
                    const canRemoveAny = isStaffOrAdmin && !isMainReserverUser;
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {isParticipant && !isMainReserver && (
              <button
                onClick={() => removeParticipant(user.id_number)}
                disabled={removingParticipant === user.id_number}
                className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
              >
                {removingParticipant === user.id_number ? (
                  <>
                    <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
