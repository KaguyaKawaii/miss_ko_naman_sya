...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  // NEW: Remove participant function
  const removeParticipant = async (participantId) => {
    if (!localReservation || !participantId) return;
    
    setProcessingAction("remove-participant");
    
    try {
      const response = await axios.delete(
        `${import.meta.env.VITE_API_URL}/api/reservations/${localReservation._id}/participants/${participantId}`
      );
  ...
  // Get main reserver info
  const getMainReserver = () => {
    if (!localReservation) return null;
    
    if (typeof localReservation.userId === 'object') {
      return {
        id_number: localReservation.userId.id_number,
        name: localReservation.userId.name,
        course: localReservation.userId.course || "N/A",
        year_level: localReservation.userId.yearLevel || "N/A",
        department: localReservation.userId.department || "N/A",
        email: localReservation.userId.email || "N/A"
      };
    }
  ...
  // Filter out main reserver from participants
  const getFilteredParticipants = () => {
    if (!localReservation || !localReservation.participants) return [];
    
    const mainReserverIdNumber = getMainReserver()?.id_number;
    
    return localReservation.participants.filter(participant => 
      participant.id_number !== mainReserverIdNumber &&
      participant.idNumber !== mainReserverIdNumber
    );
  };
  ...
  const mainReserver = getMainReserver();

  // NEW: Check if participant removal is allowed
  const canRemoveParticipants = isMainReserver && 
  ...
  return (
    <main className="ml-0 lg:ml-[250px] w-full lg:w-[calc(100%-250px)] flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="p-4 lg:p-6 space-y-6">
        ...
        {/* Main Reserver Information */}
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100">
          <h2 className="text-lg font-semibold text-gray-800 mb-4 pb-2 border-b border-gray-100">Main Reserver</h2>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                <tr className="bg-white hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm text-gray-500">
                    {mainReserver.year_level}
                  </td>
                  <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm text-gray-500">
                    {mainReserver.department}
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        {/* Participants Table (excluding main reserver) */}
            ...
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100">
              ...
              {canRemoveParticipants && (
                <p className="text-sm text-gray-500">
                  You can remove participants who cancelled
                </p>
              )}
            </div>
            <div className="overflow-x-auto rounded-lg border border-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredParticipants.map((participant, index) => (
                    <tr key={index} className="bg-white hover:bg-gray-50">
                      <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm font-medium text-gray-900">
                        {participant.id_number || participant.idNumber}
                      </td>
                      <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm text-gray-900">
                        {participant.name}
                      </td>
                      ...
                      <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm text-gray-500">
                        {participant.year_level || participant.yearLevel || "N/A"}
                      </td>
                      <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm text-gray-500">
                      ...
                      {canRemoveParticipants && (
                        <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm">
                          <button
                            ...
                            disabled={processingAction === "remove-participant"}
                            className="text-red-600 hover:text-red-800 disabled:text-red-300 disabled:cursor-not-allowed cursor-pointer flex items-center"
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {isStaffOrAdmin && localReservation.status === "Pending" && (
              <>
                <button
                  ...
                  className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors disabled:bg-green-400 disabled:cursor-not-allowed cursor-pointer flex items-center"
                >
                ...
                <button
                  ...
                  className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors disabled:bg-red-400 disabled:cursor-not-allowed cursor-pointer flex items-center"
                >
            ...
            {isStaffOrAdmin && localReservation.status === "Approved" && (
              <button
                onClick={() => setShowStartConfirm(true)}
                disabled={processingAction === "start"}
                className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors disabled:bg-blue-400 disabled:cursor-not-allowed cursor-pointer flex items-center"
              >
            ...
            {/* End Early Button */}
            {isStaffOrAdmin && localReservation.status === "Ongoing" && (
              <button
                onClick={() => setShowEndEarlyConfirm(true)}
                disabled={processingAction === "end-early"}
                className="bg-orange-600 text-white px-4 py-2 rounded-lg hover:bg-orange-700 transition-colors disabled:bg-orange-400 disabled:cursor-not-allowed cursor-pointer flex items-center"
              >
            ...
            {/* Extension Approval/Rejection for Staff/Admin */}
            ...
            {/* Cancel Button for Main Reserver */}
            {(isMainReserver && (localReservation.status === "Pending" || localReservation.status === "Approved")) && (
              <button
      ...
      {/* NEW: Remove Participant Confirmation Modal */}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            ...
            <p className="text-gray-600 mb-4">
              Are you sure you want to remove <strong>{participantToRemove.name}</strong> from this reservation?
            </p>
            <div className="flex justify-end space-x-3">
              ...
              <button
                onClick={() => removeParticipant(participantToRemove.id_number || participantToRemove.idNumber)}
                disabled={processingAction === "remove-participant"}
                className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors disabled:bg-red-400 disabled:cursor-not-allowed cursor-pointer"
              >
      ...
      {showStartConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">Start Session</h3>
            <p className="text-gray-600 mb-6">Are you sure you want to start this reservation session?</p>
            <div className="flex justify-end space-x-3">
              <button
              ...
              <button
                ...
                disabled={processingAction === "start"}
                className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors disabled:bg-blue-400 disabled:cursor-not-allowed cursor-pointer"
              >
      ...
      {showEndEarlyConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">End Session Early</h3>
            <p className="text-gray-600 mb-6">Are you sure you want to end this session early?</p>
            <div className="flex justify-end space-x-3">
              <button
              ...
              <button
                ...
                disabled={processingAction === "end-early"}
                className="bg-orange-600 text-white px-4 py-2 rounded-lg hover:bg-orange-700 transition-colors disabled:bg-orange-400 disabled:cursor-not-allowed cursor-pointer"
              >
      ...
      {showApproveConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            ...
            <p className="text-gray-600 mb-6">Are you sure you want to approve this reservation?</p>
            <div className="flex justify-end space-x-3">
              <button
              ...
              <button
                ...
                disabled={processingAction === "approve"}
                className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors disabled:bg-green-400 disabled:cursor-not-allowed cursor-pointer"
              >
      ...
      {showRejectConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            ...
            <p className="text-gray-600 mb-6">Are you sure you want to reject this reservation?</p>
            <div className="flex justify-end space-x-3">
              <button
      ...
      {showApproveExtensionConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            ...
            <p className="text-gray-600 mb-6">Are you sure you want to approve this extension request?</p>
            <div className="flex justify-end space-x-3">
              <button
      ...
      {showRejectExtensionConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            ...
            <p className="text-gray-600 mb-6">Are you sure you want to reject this extension request?</p>
            <div className="flex justify-end space-x-3">
              <button
      ...
      {showExtendModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Request Extension</h3>
            <p className="text-gray-600 mb-4">
              Requesting an extension will automatically extend your reservation time by 30 minutes, subject to approval and room availability.
            </p>
            {conflictInfo && conflictInfo.hasConflict && (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">
              {modalMessage.includes("successfully") || modalMessage.includes("approved") ? "Success" : "Notice"}
            </h3>
