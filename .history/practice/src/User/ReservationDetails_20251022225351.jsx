...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  // Check if current user is a participant in this reservation
  const isParticipant = user && localReservation?.participants?.some(
    participant => participant.id_number === user.id_number
  );

  // Check if current user can remove a specific participant
  const canRemoveParticipant = (participant) => {
    if (!user || !localReservation) return false;
    
    // Main reserver can remove any participant (except themselves)
    if (isMainReserver && participant.id_number !== user.id_number) {
      return true;
    }
    
    // Participants can only remove themselves
    if (isParticipant && participant.id_number === user.id_number) {
      return true;
    }
    ...
    try {
      ...
      switch (action) {
        ...
        case "remove-participant":
          endpoint = `${import.meta.env.VITE_API_URL}/api/reservations/${localReservation._id}/remove-participant`;
          method = "put";
          requestData = { 
            participantIdNumber: data.participantIdNumber,
            isSelfRemoval: data.isSelfRemoval || false
          };
          break;
  ...
  const handleRemoveParticipant = (participant) => {
    ...
    if (participant.id_number === user.id_number) {
      // Self-removal (leaving reservation)
      setShowLeaveConfirm(true);
    } else {
      // Main reserver removing another participant
  ...
  const confirmRemoveParticipant = async () => {
    ...
    const isSelfRemoval = participantToRemove.id_number === user.id_number;
    
    await handleAction("remove-participant", {
      participantIdNumber: participantToRemove.id_number,
  ...
  // Check if participant removal is allowed (only for Pending or Approved status)
  const canRemoveParticipants = ["Pending", "Approved"].includes(localReservation.status);
  ...
  return (
    <main className="ml-0 lg:ml-[250px] w-full lg:w-[calc(100%-250px)] flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="p-4 lg:p-6 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100">
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100">
              ...
              {!canRemoveParticipants && (
                <div className="text-sm text-gray-500 bg-yellow-50 px-3 py-1 rounded-lg border border-yellow-200">
                  Participant changes not allowed for {localReservation.status} reservations
                </div>
              )}
            </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {localReservation.participants.map((participant, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-sm font-medium text-gray-900">
                        {participant.id_number}
                      </td>
                      <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-sm text-gray-800">
                        {participant.name}
                      </td>
                      <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-sm text-gray-800">
                        {participant.course}
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {isStaffOrAdmin && localReservation.status === "Pending" && (
              <>
                <button
                  ...
                  className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                >
            ...
            {hasPendingExtension && isStaffOrAdmin && (
              <>
                <button
              ...
              <>
                ...
                {localReservation.status === "Ongoing" && (
                  <button
                    ...
                    className="bg-orange-600 text-white px-4 py-2 rounded-lg hover:bg-orange-700 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                  >
          ...
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                disabled={cancelling}
                className="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 transition-colors cursor-pointer disabled:opacity-50"
              >
      ...
      {showStartConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Start Reservation</h3>
      ...
      {showEndEarlyConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">End Reservation Early</h3>
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                disabled={processingAction === "end-early"}
                className="bg-orange-600 text-white px-4 py-2 rounded hover:bg-orange-700 transition-colors cursor-pointer disabled:opacity-50"
              >
                {processingAction === "end-early" ? "Ending..." : "Yes, End Now"}
              </button>
            </div>
          </div>
        </div>
      ...
      {showApproveConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Approve Reservation</h3>
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                disabled={processingAction === "approve"}
                className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition-colors cursor-pointer disabled:opacity-50"
              >
      ...
      {showRejectConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Reject Reservation</h3>
      ...
      {showApproveExtensionConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Approve Extension</h3>
      ...
      {showRejectExtensionConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Reject Extension</h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-4">
              Are you sure you want to remove {participantToRemove?.name} ({participantToRemove?.id_number}) from this reservation?
            </p>
            <div className="flex justify-end space-x-3">
              <button
                onClick={() => setShowRemoveParticipantConfirm(false)}
                className="px-4 py-2 text-gray-600 hover:text-gray-800 cursor-pointer"
              >
                Cancel
              </button>
      ...
      {showLeaveConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Leave Reservation</h3>
            <p className="text-gray-600 mb-4">
              Are you sure you want to leave this reservation? You will no longer be listed as a participant.
            </p>
            <div className="flex justify-end space-x-3">
              <button
              ...
              <button
                ...
                className="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 transition-colors cursor-pointer disabled:opacity-50"
              >
                {processingAction === "remove-participant" ? "Leaving..." : "Yes, Leave"}
              </button>
            </div>
          </div>
        </div>
      ...
      {showExtendModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Request Extension</h3>
            <p className="text-gray-600 mb-4">
              You are about to request an extension for this reservation. The system will automatically find the latest possible end time based on room availability.
            </p>
            <div className="flex justify-end space-x-3">
              <button
              ...
              <button
                ...
                className="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700 transition-colors cursor-pointer disabled:opacity-50"
              >
                {processingAction === "request-extension" ? "Requesting..." : "Request Extension"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">
              {modalMessage.includes("successfully") || modalMessage.includes("approved") || modalMessage.includes("completed") 
                ? "Success" 
                : "Notice"}
            </h3>
            ...
            <div className="flex justify-end">
              <button
                ...
                className="bg-[#CC0000] text-white px-4 py-2 rounded hover:bg-red-700 transition-colors cursor-pointer"
              >
                OK
              </button>
            </div>
