...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  // Add to ReservationDetails.jsx component

const [showManageParticipants, setShowManageParticipants] = useState(false);
const [newParticipantId, setNewParticipantId] = useState("");

const handleRemoveParticipant = async (participantIdNumber) => {
  ...
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_API_URL}/api/reservations/${localReservation._id}/remove-participant`,
      {
        participantIdNumber,
    ...
    setModalMessage(err.response?.data?.message || "Failed to remove participant");
    setShowResultModal(true);
  } finally {
    setProcessingAction("");
  }
...
const handleAddParticipant = async () => {
  if (!localReservation || !newParticipantId) return;
  
  setProcessingAction("add-participant");
  
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_API_URL}/api/reservations/${localReservation._id}/add-participant`,
      {
        participantIdNumber: newParticipantId
      }
    );
    ...
    setModalMessage(err.response?.data?.message || "Failed to add participant");
    setShowResultModal(true);
  } finally {
    setProcessingAction("");
  }
  ...
  return (
    <main className="ml-0 lg:ml-[250px] w-full lg:w-[calc(100%-250px)] flex flex-col min-h-screen bg-gray-50">
      ...
      // Add to ReservationDetails.jsx - in the Action Buttons section

{/* Participant Management - Only for main reserver before reservation starts */}
{isMainReserver && ["Pending", "Approved"].includes(localReservation.status) && (
  <>
    <button
      onClick={() => setShowManageParticipants(true)}
      className="px-4 lg:px-5 py-2 lg:py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center cursor-pointer text-sm lg:text-base"
    >
      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 lg:h-5 lg:w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
...
// Add the modal for participant management
{showManageParticipants && (
  <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div className="bg-white rounded-2xl p-6 w-full max-w-2xl shadow-2xl border border-gray-200 max-h-[90vh] overflow-y-auto">
      ...
      <div className="mb-6">
        <h4 className="text-lg font-medium text-gray-700 mb-3">Current Participants</h4>
          ...
          {localReservation.participants.map((participant, index) => (
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
              ...
              <button
                ...
                disabled={processingAction}
                className="px-3 py-1 bg-red-100 text-red-600 rounded hover:bg-red-200 disabled:opacity-50 transition-colors cursor-pointer text-sm"
              >
                Remove
              </button>
            </div>
      ...
      <div className="border-t pt-4">
        <h4 className="text-lg font-medium text-gray-700 mb-3">Add New Participant</h4>
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={!newParticipantId || processingAction}
            className="px-4 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors cursor-pointer"
          >
            Add
          </button>
        </div>
