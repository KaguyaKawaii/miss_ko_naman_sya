...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  const [validatingParticipant, setValidatingParticipant] = useState(false);
  const [participantValidation, setParticipantValidation] = useState(null);
  ...
  const refreshReservationData = async () => {
    ...
    try {
      const response = await axios.get("localhost:5000/api/reservations/${localReservation._id}");
  ...
  const handleAction = async (action, data = {}) => {
    ...
    try {
      ...
      switch (action) {
        case "approve":
          endpoint = "localhost:5000/api/reservations/${localReservation._id}/status";
          method = "patch";
        ...
        case "start":
          endpoint = "localhost:5000/api/reservations/start/${localReservation._id}";
          method = "post";
          break;

        case "end-early":
          // FIXED: Use the correct endpoint for ending early
          endpoint = "localhost:5000/api/reservations/${localReservation._id}/end-early";
          method = "post"; // Changed from put to post if that's what your backend expects
          break;

        case "cancel":
          endpoint = "localhost:5000/api/reservations/${localReservation._id}";
          method = "delete";
          break;

        case "request-extension":
          endpoint = "localhost:5000/api/reservations/${localReservation._id}/request-extension";
          method = "put";
          requestData = { 
        ...
        case "approve-extension":
          endpoint = "localhost:5000/api/reservations/${localReservation._id}/handle-extension";
          method = "put";
  ...
  const validateParticipantId = async (idNumber) => {
    if (!idNumber.trim()) {
      setParticipantValidation({ valid: false, message: "ID number is required" });
      return;
    }

    setValidatingParticipant(true);
    try {
      const response = await axios.post("localhost:5000/api/reservations/validate-participant-id", {
        idNumber: idNumber.trim(),
        reservationId: localReservation._id
      });
      
      setParticipantValidation(response.data);
    } catch (error) {
      console.error("Error validating participant ID:", error);
      setParticipantValidation({
        valid: false,
        message: error.response?.data?.message || "Error validating ID number"
      });
    } finally {
      setValidatingParticipant(false);
    }
  };

  const handleAddParticipant = async () => {
    if (!participantValidation?.valid || !newParticipantId.trim()) {
      setModalMessage("Please validate a valid participant ID first.");
      setShowResultModal(true);
      return;
    }
    ...
    try {
      const response = await axios.post(
        "localhost:5000/api/reservations/${localReservation._id}/add-participant",
        {
          userId: user._id,
          participantIdNumber: newParticipantId.trim()
        }
      );
      ...
      setModalMessage(error.response?.data?.message || "Failed to add participant");
      setShowResultModal(true);
    } finally {
  ...
  const handleRemoveParticipant = async () => {
    ...
    try {
      const response = await axios.post(
        "localhost:5000/api/reservations/${localReservation._id}/remove-participant/${participantToRemove.id_number}",
        {
          userId: user._id
        }
      );
      ...
      setModalMessage(error.response?.data?.message || "Failed to remove participant");
      setShowResultModal(true);
    } finally {
      setManagingParticipants(false);
    }
  ...
  const openRemoveParticipantModal = (participant) => {
  ...
  const canManageParticipants = isMainReserver && 
                               ["Pending", "Approved"].includes(localReservation.status) &&
                               localReservation.participants.length < 7; // Max 7 participants + 1 main user
        ...
        {localReservation.participants && localReservation.participants.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100">
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100">
              <h2 className="text-lg font-semibold text-gray-800">Participants ({localReservation.participants.length + 1})</h2>
              {canManageParticipants && (
                <div className="flex gap-2">
                  <button
                    onClick={openAddParticipantModal}
                    disabled={localReservation.participants.length >= 7}
                    className="px-3 py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors flex items-center cursor-pointer text-sm"
                  >
            ...
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex justify-between items-center">
                <div>
                  <p className="font-medium text-blue-800 text-sm">Main Reserver</p>
                  <p className="text-blue-700 text-sm">
                    {localReservation.userId?.name || "N/A"} ({localReservation.userId?.id_number || "N/A"})
                  </p>
                </div>
                <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded">Main</span>
              </div>
            </div>
                    ...
                    <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50 hover:bg-gray-100"}>
                      ...
                      {canManageParticipants && (
                        <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm">
                          <button
                            onClick={() => openRemoveParticipantModal(p)}
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-200">
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <button
                    onClick={() => validateParticipantId(newParticipantId)}
                    disabled={!newParticipantId.trim() || validatingParticipant}
                    className="px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors flex items-center cursor-pointer"
                  >
              ...
              {participantValidation && (
                <div className={`p-3 rounded-lg border ${
                  ...
                  <p className={`text-sm font-medium ${
                    participantValidation.valid ? 'text-green-800' : 'text-red-800'
                  }`}>
                    {participantValidation.valid ? (
                      <span className="flex items-center">
                        ...
                        Valid participant: {participantValidation.user?.name}
                      </span>
                    ) : (
                      participantValidation.message
                    )}
                  </p>
                  {participantValidation.valid && participantValidation.user && (
                    <div className="mt-2 text-sm text-green-700">
                      <p><strong>Course:</strong> {participantValidation.user.course}</p>
                      <p><strong>Department:</strong> {participantValidation.user.department}</p>
                      <p><strong>Year Level:</strong> {participantValidation.user.year_level}</p>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <p className="text-sm text-blue-800 font-medium mb-1">Group Size Information</p>
                <p className="text-sm text-blue-700">
                  Current group: {localReservation.participants.length + 1}/8 users
                  {localReservation.participants.length >= 7 && (
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={!participantValidation?.valid || managingParticipants}
                className="px-6 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors flex items-center cursor-pointer font-medium"
              >
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                <p className="text-sm text-yellow-800">
                  <strong>Note:</strong> The participant will be notified about their removal from this reservation.
                </p>
              </div>
            </div>
