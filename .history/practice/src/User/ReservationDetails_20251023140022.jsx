...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  const validateParticipantId = async (idNumber) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/reservations/validate-participant-id", {
  ...
  const handleAddParticipant = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/reservations/${localReservation._id}/add-participant",
        {
          userId: user._id,
...
// FIXED: Added missing handleRemoveParticipant function with proper URL interpolation
const handleRemoveParticipant = async (participantId) => {
  try {
    console.log('Removing participant ID:', participantId, 'from reservation:', localReservation._id);
    
    // Make sure participantId is actually the ID number string, not an event object
    if (typeof participantId !== 'string' && typeof participantId !== 'number') {
            ...
            <div className="overflow-x-auto rounded-lg border border-gray-200">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  // In your participants table or list, make sure you're passing the participant object
{localReservation.participants.map((participant, i) => (
  <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50 hover:bg-gray-100"}>
    <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm font-medium text-gray-900">
      {participant.id_number || participant.idNumber || "N/A"}
    </td>
    ...
    <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm text-gray-500">
      {/* ✅ FIXED: Pass the participant's ID number */}
      <button 
        onClick={() => handleRemoveParticipant(participant.id_number || participant.idNumber)}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-200">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={() => handleRemoveParticipant(participant.id_number)}
