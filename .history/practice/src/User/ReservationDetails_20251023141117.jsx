...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  const [validatedParticipant, setValidatedParticipant] = useState(null);
...
const validateParticipantId = async (idNumber) => {
  ...
  try {
    ...
    // ✅ ADD THIS LINE to store the validated user
    if (response.data.valid && response.data.user) {
      setValidatedParticipant(response.data.user);
    } else {
      setValidatedParticipant(null);
    }
  } catch (error) {
    ...
    setValidatedParticipant(null); // ✅ Reset on error
  } finally {
...
const handleAddParticipant = async (participantId) => {
  try {
    console.log('Adding participant ID:', participantId);
    
    // Make sure participantId is actually a string/number, not an event object
    if (typeof participantId !== 'string' && typeof participantId !== 'number') {
      console.error('Invalid participantId type:', typeof participantId, participantId);
      return;
    }
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-200">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={() => handleAddParticipant(validatedParticipant.id_number)}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-200">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                onClick={() => handleRemoveParticipant(participantToRemove.id_number)}
