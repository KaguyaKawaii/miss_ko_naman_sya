...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  // New states for participant validation
  ...
  const validateParticipant = async () => {
    if (!newParticipantId.trim() || !localReservation) return;
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/reservations/validate-participant`, {
        idNumber: newParticipantId,
      ...
      if (response.data.valid) {
        setModalMessage(`Participant "${response.data.user.name}" can be added to the reservation.`);
        setShowResultModal(true);
        // You could automatically add the participant here or show an "Add" button
      } else {
        setParticipantError(response.data.message);
      }
    } catch (err) {
      console.error("Error validating participant:", err);
      setParticipantError(err.response?.data?.message || "Error validating participant ID");
    } finally {
  ...
  // Check if user can add participants (main reserver for pending/approved reservations)
  const canAddParticipants = isMainReserver && 
          ...
          <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100">
            ...
            {canAddParticipants && (
              <button
                ...
                className="px-4 py-2 bg-[#FFCC00] text-gray-800 rounded-lg hover:bg-yellow-500 transition-colors flex items-center cursor-pointer text-sm"
              >
          ...
          {localReservation.participants && localReservation.participants.length > 0 ? (
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              {canAddParticipants && (
                <button
                  onClick={handleAddParticipant}
                  className="mt-3 px-4 py-2 bg-[#FFCC00] text-gray-800 rounded-lg hover:bg-yellow-500 transition-colors cursor-pointer"
                >
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-200">
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#FFCC00] focus:border-[#FFCC00]"
                  onKeyPress={(e) => {
                    if (e.key === 'Enter') {
                      validateParticipant();
                    }
              ...
              {validationResult && validationResult.valid && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <div className="flex items-center">
                    ...
                    <span className="text-green-800 font-medium">Valid Participant</span>
                  </div>
                  <div className="mt-2 text-sm text-green-700">
                    <p><strong>Name:</strong> {validationResult.user.name}</p>
                    <p><strong>Course:</strong> {validationResult.user.course}</p>
                    <p><strong>Department:</strong> {validationResult.user.department}</p>
                  </div>
                </div>
              )}
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={validatingParticipant || !newParticipantId.trim()}
                className="px-6 py-2.5 bg-[#FFCC00] text-gray-800 rounded-lg hover:bg-yellow-500 disabled:opacity-50 transition-colors flex items-center cursor-pointer font-medium"
              >
