...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
    ...
    try {
      ...
      switch (action) {
        ...
        case "add-participant":
          endpoint = `http://localhost:5000/api/reservations/${localReservation._id}/add-participant`;
          method = "put";
          requestData = { participant: data.participant };
          break;
  ...
  const fetchUserByID = async () => {
    if (!idNumber.trim()) {
      setModalMessage("Please enter an ID Number");
      setShowResultModal(true);
      return;
    }
    ...
    try {
      const response = await axios.get(`http://localhost:5000/api/reservations/user/${userId}/info`);
      setFetchedUser(response.data);
    } catch (err) {
      console.error('Error fetching user:', err);
      const errorMessage = err.response?.data?.message || 
                          err.response?.data?.error || 
                          'User not found with this ID Number';
      setModalMessage(errorMessage);
      setShowResultModal(true);
      setFetchedUser(null);
    } finally {
  ...
  const confirmAddParticipant = async () => {
    ...
    if (localReservation.participants && localReservation.participants.length >= 4) {
      setModalMessage("Maximum of 4 participants reached. Cannot add more participants.");
      setShowResultModal(true);
      return;
    }
    ...
    if (!fetchedUser) {
      setModalMessage("Please search for a user first using their ID Number");
      setShowResultModal(true);
      return;
    }

    // Check if user is already a participant
    const isAlreadyParticipant = localReservation.participants?.some(
      p => p.id_number === fetchedUser.id_number || p._id === fetchedUser._id
    );

    if (isAlreadyParticipant) {
      setModalMessage("This user is already a participant in this reservation.");
      setShowResultModal(true);
      return;
    }
    ...
    await handleAction("add-participant", {
  ...
  const handleResultModalClose = () => {
    ...
    // Only go back to dashboard if the reservation was deleted or cancelled
  ...
  const canAddParticipant = isMainReserver && localReservation.participants && localReservation.participants.length < 4;
          ...
          <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100">
            ...
            <div className="flex items-center gap-4">
              <div className="text-sm text-gray-500 bg-blue-50 px-3 py-1 rounded-lg border border-blue-200">
                {localReservation.participants ? localReservation.participants.length : 0} / 4 participants
              </div>
              {canAddParticipant && (
                <button
                  onClick={handleAddParticipant}
                  className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center cursor-pointer text-sm"
                >
          ...
          {localReservation.participants && localReservation.participants.length > 0 ? (
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              {canAddParticipant && (
                <button
                  onClick={handleAddParticipant}
                  className="mt-3 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors cursor-pointer"
                >
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-200">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to remove <strong>{participantToRemove?.name || 'this participant'}</strong> from the reservation?
            </p>
            <div className="flex justify-end gap-3">
              <button
                onClick={() => {
              ...
              <button
                ...
                className="px-6 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 transition-colors flex items-center cursor-pointer font-medium"
              >
                {processingAction === "remove-participant" ? (
                  <>
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                    Removing...
                  </>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-200">
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={fetchingUser || !idNumber.trim()}
                    className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors cursor-pointer whitespace-nowrap"
                  >
                    {fetchingUser ? (
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-gray-800">{fetchedUser.name}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">ID Number:</span>
                      <span className="font-medium text-gray-800">{fetchedUser.id_number}</span>
                    </div>
                    {fetchedUser.course && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Course:</span>
                        <span className="font-medium text-gray-800">{fetchedUser.course}</span>
                      </div>
                    )}
                    {fetchedUser.year_level && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Year Level:</span>
                        <span className="font-medium text-gray-800">{fetchedUser.year_level}</span>
                      </div>
                    )}
                    {fetchedUser.department && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Department:</span>
                        <span className="font-medium text-gray-800">{fetchedUser.department}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <p className="text-sm text-blue-700">
                  <strong>Note:</strong> Maximum of 4 participants allowed per reservation. Currently: {localReservation.participants ? localReservation.participants.length : 0}/4
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={processingAction === "add-participant" || !fetchedUser}
                className="px-6 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors flex items-center cursor-pointer font-medium"
              >
                {processingAction === "add-participant" ? (
                  <>
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                    Adding...
                  </>
