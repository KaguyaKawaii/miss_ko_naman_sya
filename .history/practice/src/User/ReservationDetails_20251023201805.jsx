...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  // New function to handle participant removal
  const handleRemoveParticipant = async () => {
    if (!localReservation || !participantToRemove) return;
    
    setRemovingParticipant(true);

    try {
      const response = await axios.delete(
        `http://localhost:5000/api/reservations/${localReservation._id}/participants/${participantToRemove._id || participantToRemove.id}`
      );
  ...
  // Function to show remove confirmation modal
  const showRemoveConfirm = (participant) => {
    setParticipantToRemove(participant);
    setShowRemoveParticipantConfirm(true);
  };

  // New function to handle adding participant
  ...
  // Function to handle input changes for new participant
  ...
  // Add this to your newParticipant state initialization
const [newParticipant, setNewParticipant] = useState({
  id_number: "",
  name: "",
  course: "",
  ...
  if (!newParticipant.id_number.trim()) return;

  try {
    setAddingParticipant(true);
    
    // Call your API to fetch participant details by ID number
    const response = await axios.get(
      `${import.meta.env.VITE_API_URL}/api/participants/${newParticipant.id_number}`
    );
    ...
    setNewParticipant(prev => ({
      ...
      year_level: participantData.year_level || participantData.yearLevel || "",
    ...
    // If participant not found, keep the form as is and let user fill manually
    if (error.response?.status === 404) {
      // Optional: Show a message that participant was not found
      setModalMessage("Participant not found. Please fill in the details manually.");
      setShowResultModal(true);
    } else {
      setModalMessage("Error fetching participant details. Please fill in the details manually.");
      setShowResultModal(true);
    }
  } finally {
    setAddingParticipant(false);
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100">
            ...
            {localReservation.participants.length > 0 ? (
                      ...
                      <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50 hover:bg-gray-100"}>
                        ...
                        {(isMainReserver || isStaffOrAdmin) && (
                          <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm">
                            <button
                              onClick={() => showRemoveConfirm(p)}
                              className="text-red-600 hover:text-red-800 hover:bg-red-50 px-2 py-1 rounded text-xs lg:text-sm transition-colors cursor-pointer"
                            >
                              Remove
                            </button>
                          </td>
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                {(isMainReserver || isStaffOrAdmin) && (
                  <p className="text-sm mt-2">Click "Add Participant" to add participants to this reservation.</p>
                )}
              </div>
            )}
          </div>
  ...
  <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-200">
      ...
      <div className="space-y-4 mb-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Participant ID Number</label>
          <input
            name="id_number"
            type="text"
            value={newParticipant.id_number}
            onChange={handleNewParticipantChange}
            onBlur={fetchParticipantDetails} // Fetch details when user leaves the field
            placeholder="Enter ID number"
            className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
          />
          <p className="text-xs text-gray-500 mt-1">Enter ID number and other details will auto-fill</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input
              name="year_level"
              type="text"
              value={newParticipant.year_level}
              onChange={handleNewParticipantChange}
      ...
      <div className="flex justify-end gap-3">
        ...
        <button
          ...
          disabled={addingParticipant || !newParticipant.id_number.trim() || !newParticipant.name.trim()}
