...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  // Fetch participant details by ID number
  ...
  const handleAddParticipant = async () => {
    if (!newParticipant.id_number.trim() || !newParticipant.name.trim()) {
      setModalMessage("ID number and name are required.");
      setShowResultModal(true);
      return;
    }
    ...
    try {
      ...
      // Call API to add participant to reservation
      ...
      <div className="p-4 lg:p-6 space-y-6">
        ...
        {localReservation.participants && localReservation.participants.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100">
            ...
            {isMainReserver && localReservation.status === "Ongoing" && (
              <div className="mt-4 flex justify-end">
                <button
                  onClick={() => setShowAddParticipantModal(true)}
                  className="px-4 lg:px-5 py-2 lg:py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center cursor-pointer text-sm lg:text-base"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 lg:h-5 lg:w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0l6-16" />
                  </svg>
                  Add Participant
                </button>
              </div>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <input
                  ...
                  onBlur={fetchParticipantDetails}
