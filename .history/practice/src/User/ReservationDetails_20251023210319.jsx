...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  // Use useRef for the interval instead of state to prevent re-renders
  ...
  // Refresh reservation data function with useCallback to prevent unnecessary recreations
  const refreshReservationData = useCallback(async () => {
    console.log('Auto-refresh triggered at:', new Date().toLocaleTimeString());
    if (!localReservation?._id) {
      console.log('No localReservation ID found');
      return;
    }

    try {
      console.log('Fetching updated reservation data for ID:', localReservation._id);
  ...
  }, [localReservation?._id, refreshReservations]);

  // Auto-refresh functionality for real-time updates
  useEffect(() => {
    if (!localReservation) {
      console.log('No localReservation, skipping auto-refresh setup');
      return;
    }

    console.log('Setting up auto-refresh interval for reservation:', localReservation._id);
  ...
  }, [localReservation, refreshReservationData]);

  // Cleanup when reservation becomes invalid
  useEffect(() => {
    if (!localReservation && refreshIntervalRef.current) {
      console.log('Reservation became invalid, clearing interval');
      clearInterval(refreshIntervalRef.current);
      refreshIntervalRef.current = null;
    }
  }, [localReservation]);
