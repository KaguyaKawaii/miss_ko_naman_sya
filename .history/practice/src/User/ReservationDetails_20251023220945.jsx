...
// Initialize modal with current participants
const openEditParticipantsModal = () => {
  setShowEditParticipantsModal(true);
  if (!localReservation) return;
  
  // Get current participant IDs (excluding main user)
  const currentIds = localReservation.participants
    .filter(p => p.id_number !== localReservation.userId.id_number)
    .map(p => p.id_number);
...
const addParticipant = () => {
  ...
  if (participantIds.includes(id)) {
    setParticipantError(`Participant with ID ${id} is already in the list`);
    return;
  }
  
  // Check group size limit (max 7 participants + 1 main user = 8 total)
  if (participantIds.length >= 7) {
    setParticipantError('Maximum 7 participants allowed (8 total including you)');
    return;
  }
...
const removeParticipant = (idToRemove) => {
  setParticipantIds(participantIds.filter(id => id !== idToRemove));
  setParticipantError('');
};

// Update participants on server
const handleUpdateParticipants = async () => {
  if (!localReservation) return;
  
  // Validate group size
  const totalGroupSize = participantIds.length + 1;
  if (totalGroupSize < 4) {
    setParticipantError('At least 3 participants are required (total group size 4 including you)');
    return;
  }
  ...
  try {
    ...
    if (response.data.success) {
      setParticipantSuccess('Participants updated successfully!');
      ...
      setParticipantError(response.data.message || 'Failed to update participants');
      
      // Show detailed validation errors if available
      if (response.data.invalidParticipants) {
    ...
    if (error.response?.data?.invalidParticipants) {
      setInvalidParticipants(error.response.data.invalidParticipants);
    }
  } finally {
...
{isMainReserver && ["Pending", "Approved"].includes(localReservation?.status) && (
  <button
    onClick={openEditParticipantsModal}  // Changed from setShowEditParticipantsModal(true)
    disabled={updatingParticipants}
    className="px-4 lg:px-5 py-2 lg:py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors flex items-center cursor-pointer text-sm lg:text-base"
  >
    ...
    {updatingParticipants ? "Updating..." : "Edit Participants"}
  </button>
)}
            </div>
    ...
    <div className="bg-white rounded-2xl p-6 w-full max-w-2xl shadow-2xl border border-gray-200 max-h-[90vh] overflow-y-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center">
          ...
          <h3 className="text-xl font-semibold text-gray-800">Edit Participants</h3>
        </div>
        <button
      ...
      <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <span className="text-blue-600">Current Participants:</span>
            <span className="ml-2 font-medium">{localReservation.participants?.length || 0}</span>
          </div>
          <div>
            <span className="text-blue-600">Total Group Size:</span>
            <span className="ml-2 font-medium">{(localReservation.participants?.length || 0) + 1} / 8</span>
          </div>
          <div className="col-span-2">
            <span className="text-blue-600">Requirements:</span>
            <span className="ml-2">Minimum 3 participants, maximum 7 participants (4-8 total including you)</span>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-3">
          Current Participants ({participantIds.length})
        </label>
      ...
      {participantError && (
        <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
          <div className="flex items-center">
            ...
            <span className="text-red-800 font-medium">{participantError}</span>
          </div>
          {invalidParticipants.length > 0 && (
            <div className="mt-2 text-sm text-red-700">
              <p className="font-medium">Invalid participants:</p>
              <ul className="list-disc list-inside mt-1">
                {invalidParticipants.map((p, i) => (
                  <li key={i}>
                    {p.name || p.identifier} - {p.reason}
                  </li>
                ))}
              </ul>
            </div>
      ...
      {participantSuccess && (
        <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-center">
            ...
            <span className="text-green-800 font-medium">{participantSuccess}</span>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
        ...
        <button
          ...
          disabled={updatingParticipants || participantIds.length === 0}
          className="px-6 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center cursor-pointer font-medium"
        >
