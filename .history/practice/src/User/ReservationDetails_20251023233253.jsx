...
const [showParticipantResultModal, setShowParticipantResultModal] = useState(false);
  ...
  try {
    ...
    switch (action) {
      case "approve":
        endpoint = `/api/reservations/${localReservation._id}/status`;
        method = "patch";
      ...
      case "start":
        endpoint = `/api/reservations/start/${localReservation._id}`;
        method = "post";
        break;

      case "end-early":
        endpoint = `/api/reservations/${localReservation._id}/end-early`;
        method = "post";
        break;

      case "cancel":
        endpoint = `/api/reservations/${localReservation._id}`;
        method = "delete";
        break;

      case "request-extension":
        endpoint = `/api/reservations/${localReservation._id}/request-extension`;
        method = "put";
        requestData = { 
      ...
      case "approve-extension":
        endpoint = `/api/reservations/${localReservation._id}/handle-extension`;
        method = "put";
    ...
    // FIX: Use relative URLs or check if VITE_API_URL is defined
  ...
  const formatDateTime = (datetime) =>
    ...
    // Add this function to load participants data
const loadParticipantsData = async () => {
  ...
  try {
    ...
    setParticipantActionMessage("Failed to load participants data");
    setShowParticipantResultModal(true);
  }
};

// Add this useEffect to load participants data when reservation changes
useEffect(() => {
  if (localReservation) {
...
const handleAddParticipant = async () => {
  if (!newParticipantId.trim()) {
    setParticipantActionMessage("Please enter an ID number");
    setShowParticipantResultModal(true);
    return;
  }
  ...
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_API_URL}/api/reservations/${localReservation._id}/participants`,
      { id_number: newParticipantId.trim() }
    );

    setParticipantActionMessage(response.data.message);
    setNewParticipantId("");
    ...
    setParticipantActionMessage(errorMessage);
  } finally {
...
const confirmRemoveParticipant = async () => {
  ...
  try {
    const response = await axios.delete(
      `${import.meta.env.VITE_API_URL}/api/reservations/${localReservation._id}/participants`,
      { data: { id_number: removingParticipant.id_number } }
    );
  ...
  return (
    <main className="ml-0 lg:ml-[250px] w-full lg:w-[calc(100%-250px)] flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="p-4 lg:p-6 space-y-6">
        ...
        {/* Participants Table */}
{/* Participants Table with Management */}
    ...
    <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100">
      ...
      {isMainReserver && ["Pending", "Approved"].includes(localReservation.status) && (
        <button
          ...
          className="px-4 py-2 bg-[#FFCC00] text-gray-800 rounded-lg hover:bg-yellow-500 transition-colors flex items-center cursor-pointer text-sm font-medium"
        >
      ...
      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex justify-between items-center text-sm">
          ...
          <span className="text-blue-700">
            {participantsData.currentGroupSize} / {participantsData.maxGroupSize} users 
            (Minimum: {participantsData.minGroupSize})
          </span>
        </div>
      </div>
    )}
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          ...
          <tr className="bg-gray-50 font-medium">
            <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm text-gray-900">
              {localReservation.userId?.id_number || "N/A"}
            </td>
            <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm text-gray-900">
              {localReservation.userId?.name || "N/A"} 
              <span className="ml-2 bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">Main Reserver</span>
            </td>
            <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm text-gray-500">
              {localReservation.userId?.course || "N/A"}
            </td>
            <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm text-gray-500">
              {localReservation.userId?.yearLevel || localReservation.userId?.year_level || "N/A"}
            </td>
            <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm text-gray-500">
              {localReservation.userId?.department || "N/A"}
            </td>
            {isMainReserver && ["Pending", "Approved"].includes(localReservation.status) && (
              <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm text-gray-500">
  ...
  <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div className="bg-white rounded-2xl p-6 w-full max-w-2xl shadow-2xl border border-gray-200 max-h-[90vh] overflow-y-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center">
          <div className="bg-[#FFCC00] p-2 rounded-full mr-3">
        ...
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-800">{participantsData.currentGroupSize}</p>
            </div>
            <div>
              <p className="text-sm text-blue-600 font-medium">Minimum Required</p>
              <p className="text-2xl font-bold text-blue-800">{participantsData.minGroupSize}</p>
            </div>
            <div>
              <p className="text-sm text-blue-600 font-medium">Maximum Allowed</p>
              <p className="text-2xl font-bold text-blue-800">{participantsData.maxGroupSize}</p>
            </div>
          </div>
          {participantsData.currentGroupSize < participantsData.minGroupSize && (
            <p className="text-sm text-red-600 mt-2 text-center">
              ❌ Need {participantsData.minGroupSize - participantsData.currentGroupSize} more participant(s) to meet minimum requirement
            </p>
          )}
        </div>
      )}
      ...
      <div className="mb-6">
        <h4 className="text-lg font-medium text-gray-800 mb-3">Add New Participant</h4>
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#FFCC00] focus:border-[#FFCC00]"
            disabled={addingParticipant}
          />
          <button
            onClick={handleAddParticipant}
            disabled={addingParticipant || !newParticipantId.trim() || (participantsData?.currentGroupSize >= 8)}
            className="px-6 py-3 bg-[#FFCC00] text-gray-800 rounded-lg hover:bg-yellow-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center cursor-pointer font-medium"
          >
            {addingParticipant ? (
              <>
        ...
        {participantsData?.currentGroupSize >= 8 && (
          <p className="text-sm text-red-600 mt-2">Maximum group size reached (8 users)</p>
        )}
      </div>

      {/* Current Participants List */}
      <div>
        <h4 className="text-lg font-medium text-gray-800 mb-3">Current Participants</h4>
        <div className="space-y-3">
          {localReservation.participants.map((participant, index) => (
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
              <div className="flex-1">
                <p className="font-medium text-gray-800">{participant.name}</p>
                <p className="text-sm text-gray-600">ID: {participant.id_number} • {participant.course}</p>
              </div>
              <button
                onClick={() => handleRemoveParticipant(participant)}
                disabled={participantsData?.currentGroupSize <= 4}
                className="text-red-600 hover:text-red-800 disabled:text-gray-400 disabled:cursor-not-allowed transition-colors cursor-pointer p-2 rounded hover:bg-red-50"
                title={participantsData?.currentGroupSize <= 4 ? "Cannot remove - minimum group size required" : "Remove participant"}
              >
...
{showRemoveConfirm && removingParticipant && (
  <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-200">
      <div className="flex items-center mb-4">
        <div className="bg-red-100 p-2 rounded-full mr-3">
      ...
      <p className="text-gray-600 mb-6">
        Are you sure you want to remove <strong>{removingParticipant.name}</strong> (ID: {removingParticipant.id_number}) from this reservation?
      ...
      <div className="flex justify-end gap-3">
        ...
        <button
          ...
          className="px-6 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors flex items-center cursor-pointer font-medium"
        >
          Yes, Remove
        </button>
      </div>
...
{/* Participant Action Result Modal */}
{showParticipantResultModal && (
  <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-200">
      ...
      <p className="text-gray-600 mb-6">{participantActionMessage}</p>
      <div className="flex justify-end">
        <button
          onClick={() => setShowParticipantResultModal(false)}
          className="px-6 py-2.5 bg-[#FFCC00] text-gray-800 rounded-lg hover:bg-yellow-500 transition-colors cursor-pointer font-medium"
        >
          OK
        </button>
      </div>
