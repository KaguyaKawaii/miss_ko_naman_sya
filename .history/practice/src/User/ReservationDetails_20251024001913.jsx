  ...
  <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100">
    ...
    <div className="overflow-x-auto rounded-lg border border-gray-200">
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {localReservation.participants
            .filter(participant => {
              // Exclude the main reserver from participants list
              // Assuming the main reserver's ID is stored in userId and participant has id_number
              const reservationUserId = getReservationUserId();
              const participantId = participant.id_number || participant.idNumber;
              
              // If we can't determine the main reserver, show all participants
              if (!reservationUserId || !participantId) return true;
              
              // Exclude participant if their ID matches the main reserver's ID
              return participantId !== reservationUserId;
            })
            .map((p, i) => (
