...
function ReserveRoom({ user, setView }) {
  ...
  const canReserveFloor = (floor) => {
    // COL and Graduate students can reserve all floors
    if (isCollegeOfLawUser() || isGraduateStudent()) {
      return true;
    }
    // Regular users can only reserve 4th and 5th floors
    return floor === "4th Floor" || floor === "5th Floor";
  };
  ...
  // Add this function to validate all participants' floor access
  const validateParticipantFloorAccess = async () => {
    try {
      const response = await axios.post("http://localhost:5000/reservations/validate-floor-access", {
        location: formData.location,
        participantIds: formData.participants.map(p => p.id_number).filter(id => id)
      });

      if (!response.data.valid) {
        const invalidNames = response.data.invalidParticipants.map(p => p.name).join(', ');
        alert(`The following participants don't have access to ${formData.location}: ${invalidNames}\n\n${response.data.restrictionMessage}`);
        return false;
      }
      return true;
    } catch (error) {
      console.error("Floor access validation error:", error);
      alert("Error validating participant access. Please try again.");
      return false;
    }
  };
  ...
  const validateForm = () => {
    ...
    if (!canReserveFloor(formData.location)) {
      if (isCollegeOfLawUser()) {
        alert("College of Law students can reserve all floors.");
      } else if (isGraduateStudent()) {
        alert("Graduate students can reserve all floors.");
      } else {
        alert("Regular students can only reserve rooms on the 4th and 5th floors.");
      }
      return false;
    }
  ...
  const submitReservation = async () => {
    ...
    // ✅ ADD THIS VALIDATION: Check if all participants have floor access
          ...
          <div className="bg-green-50 border border-green-200 rounded-xl p-4">
            <div className="flex items-center">
              ...
              <p className="text-green-800 font-medium text-sm">
                {isCollegeOfLawUser() 
                  ? "College of Law Access: You can reserve rooms on all floors."
                  : "Graduate Student Access: You can reserve rooms on all floors."}
              </p>
            </div>
          </div>
        ) : (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
            <div className="flex items-center">
              ...
              <p className="text-blue-800 font-medium text-sm">
                Regular Student Access: You can only reserve rooms on the 4th and 5th floors.
              </p>
            </div>
          </div>
        )}
            ...
            {roomLocations.map((loc) => {
              ...
              return (
                <div
                  key={loc}
                  onClick={() => {
                    if (!canReserve && !(isCollegeOfLawUser() || isGraduateStudent())) {
                      alert("Regular students can only reserve rooms on the 4th and 5th floors.");
                      return;
                    }
                    setFormData({
                  ...
                  className={`border-2 rounded-2xl w-full xs:w-[150px] sm:w-[180px] md:w-[200px] h-[120px] sm:h-[150px] md:h-[200px] flex justify-center items-center cursor-pointer transition-all duration-200 overflow-hidden relative ${
                    formData.location === loc 
                      ...
                      : !canReserve && !(isCollegeOfLawUser() || isGraduateStudent())
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`absolute inset-0 z-0 ${
                        isDisabled || (!canReserve && !(isCollegeOfLawUser() || isGraduateStudent())) ? "bg-gray-800/60" : "bg-black/30"
                      }`}></div>
                      ...
                      {/* Restricted Badge */}
                      {!canReserve && !(isCollegeOfLawUser() || isGraduateStudent()) && !isDisabled && (
                        <div className="absolute top-2 right-2 bg-red-600 text-white px-2 py-1 rounded-full text-xs font-semibold z-10">
                          Restricted
                        </div>
                      )}
