...
function ReserveRoom({ user, setView }) {
  ...
  const validateParticipantFloorAccess = async () => {
    try {
      ...
      if (!response.data.valid) {
        ...
        showAlert("Access Denied", errorMessage, "error");
        return false;
      }
      ...
      showAlert("Validation Error", `Floor access validation failed: ${errorMsg}`, "error");
      return false;
    }
  };
  ...
  const validateForm = () => {
    if (!formData.date || !formData.time || !formData.location || !formData.roomName || !formData.purpose) {
      showAlert("Incomplete Form", "Please complete all required fields.", "error");
      return false;
    }
    ...
    if (selectedRoom && !selectedRoom.isActive) {
      showAlert("Room Unavailable", "This room is currently unavailable. Please select another room.", "error");
      return false;
    }
    ...
    if (selectedDate < now) {
      showAlert("Invalid Date/Time", "You cannot reserve a room in the past. Please select a future date and time.", "error");
      return false;
    }
    ...
    if (!canReserveFloor(formData.location)) {
      if (formData.location === "Ground Floor") {
        showAlert("Access Restricted", "Ground Floor is reserved for Graduate students only.", "error");
      } else if (formData.location === "2nd Floor") {
        showAlert("Access Restricted", "2nd Floor is reserved for College of Law students only.", "error");
      } else {
        showAlert("Access Restricted", "You don't have access to this floor.", "error");
      }
      return false;
    }
    ...
    for (let i = 0; i < formData.participants.length; i++) {
      ...
      if (!p.name || !p.department || !p.id_number) {
        showAlert("Incomplete Participant", `Please complete all fields for participant ${i + 1}.`, "error");
        return false;
      }
      ...
      if (validation[i].status !== "valid") {
        showAlert("Participant Not Verified", `Participant ${i + 1} is not verified or registered.`, "error");
        return false;
      }
    }
    return true;
  ...
  const submitReservation = async () => {
    if (!user.verified) {
      setShowNotVerifiedWarning(true);
      showAlert("Account Not Verified", "You cannot reserve a room until your account is verified.", "error");
      return;
    }
    ...
    try {
      ...
      if (check.data.blocked) {
        showAlert("Reservation Limit", check.data.reason || "You have reached your reservation limit for this week.", "error");
        return;
      }
    } catch (err) {
      console.error("Limit check failed", err);
      const message = err.response?.data?.message || "Failed to verify reservation limit.";
      showAlert("Limit Check Failed", message, "error");
      return;
    }
    ...
    try {
      ...
      showAlert("Reservation Failed", error.response?.data?.message || error.message, "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  // ✅ UPDATED: Use the image from the room data assigned in AdminRooms
  const getRoomImage = (room) => {
    // If room has an image assigned from AdminRooms, use it
    if (room.image && room.image.url) {
      return room.image.url;
    }
    
    // Fallback to default images based on floor (you can remove these if you want)
    switch (room.floor) {
      case "Ground Floor":
        return "/assets/GroundFloor.jpg"; // Update path as needed
      case "5th Floor":
        return "/assets/picture2.jpg"; // Update path as needed
      default:
        return "/assets/default-room.jpg"; // Add a default room image
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        <div className={`p-6 ${
          ...
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-white">{alertTitle}</h3>
          </div>
        </div>
        <div className="p-6">
      ...
      {loading && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex flex-col items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 shadow-xl flex flex-col items-center">
            <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-[#CC0000] mb-4"></div>
            <p className="text-gray-700 text-lg font-medium">Submitting Reservation...</p>
            ...
            {roomLocations.map((loc) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="relative z-10 text-gray-800 text-sm sm:text-base md:text-lg font-semibold text-center px-2 bg-white/80 backdrop-blur-sm py-2 rounded-lg">
                    {loc}
                  </p>
                </div>
              );
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 border border-gray-100">
            ...
            <div className="flex flex-wrap gap-3 sm:gap-5 justify-center">
              {rooms
                ...
                .map((room) => {
                  // ✅ UPDATED: Get the room image from the room data
                  ...
                  return (
                    ...
                    >
                      {/* ✅ UPDATED: Use the assigned room image */}
                      {roomImage && (
                        <img
                          src={roomImage}
                          alt={room.room}
                        ...
                        <div className="relative">
                          ...
                          {validation[idx]?.loading && (
                            <div className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2">
                              <div className="animate-spin rounded-full h-3 w-3 sm:h-4 sm:w-4 border-b-2 border-[#CC0000]"></div>
                            </div>
                          )}
                        </div>
                      </td>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden text-center">
            ...
            <div className="p-6">
              ...
              <p className="text-gray-600 mb-6">
                Your reservation request has been submitted successfully. You will be redirected to the dashboard.
              </p>
              <button
                onClick={closeSuccess}
                className="w-full bg-[#CC0000] text-white py-3 rounded-lg hover:bg-red-700 transition font-medium"
              >
                Go to Dashboard
              </button>
            </div>
      ...
      {showNotVerifiedWarning && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden text-center">
            ...
            <div className="p-6">
              <h2 className="text-2xl font-bold text-gray-800 mb-2">Account Not Verified</h2>
              <p className="text-gray-600 mb-6">
