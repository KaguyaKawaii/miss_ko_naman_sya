...
const validateParticipantFloorAccess = async () => {
  ...
  // Updated getRoomImage function using shared configuration
  const getRoomImage = (room) => {
    // If room has an image object with ID, use the shared configuration
    if (room.image && room.image.id) {
      const sharedImage = getRoomImageById(room.image.id);
      if (sharedImage) return sharedImage.url;
    }
    
    // If room has an image object with URL (backward compatibility)
    if (room.image && room.image.url) {
      return room.image.url;
    }
    
    // Fallback to default images based on floor
    const floorImages = {
  ...
  const getFloorImage = (floor) => {
    const floorImageMap = {
      "Ground Floor": getRoomImageById("ground_floor")?.url,
      "5th Floor": getRoomImageById("fifth_floor")?.url,
      "Faculty Room": getRoomImageById("faculty_room")?.url,
      "Collaboration Room": getRoomImageById("collab_room")?.url,
    };
