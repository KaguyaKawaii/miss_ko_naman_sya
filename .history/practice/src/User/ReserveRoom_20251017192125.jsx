...
function ReserveRoom({ user, setView }) {
  ...
  const [showFloorAccessModal, setShowFloorAccessModal] = useState(false);
  const [floorAccessMessage, setFloorAccessMessage] = useState("");
  ...
  const validateParticipantFloorAccess = async () => {
    try {
      ...
      if (!response.data.valid) {
        ...
        setFloorAccessMessage(errorMessage);
        setShowFloorAccessModal(true);
        return false;
      }
      ...
      setFloorAccessMessage(`Floor access validation failed: ${errorMsg}`);
      setShowFloorAccessModal(true);
      return false;
    }
  };
  ...
  return (
    <main className="w-full md:ml-[250px] md:w-[calc(100%-250px)] flex flex-col bg-gray-50 min-h-screen">
      ...
      <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div>
              ...
              <button
                ...
                className="w-full p-3 border border-gray-300 rounded-lg text-left bg-white hover:bg-gray-50 transition-colors text-sm"
              >
                {formData.date ? (
            ...
            <div>
              ...
              >
                {formData.time ? (
                  <span className="text-gray-800">{formatDisplayTime(formData.time)}</span>
                ) : (
            ...
            <div>
              ...
              <button
                ...
                className="w-full p-3 border border-gray-300 rounded-lg text-left bg-white hover:bg-gray-50 transition-colors text-sm"
              >
                <span className="text-gray-800">{formData.numUsers} users</span>
              </button>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-3 border border-gray-300 rounded-lg outline-none focus:border-[#CC0000] transition-colors text-sm"
        ...
        {/* Location Selection */}
        <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 border border-gray-100">
          <h2 className="text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4 pb-2 border-b border-gray-100">Select Location</h2>
            ...
            {roomLocations.map((location) => (
              <button
                ...
                className={`p-4 rounded-xl border-2 transition-all text-left ${
                  ...
                  !canReserveFloor(location) ? "opacity-50 cursor-not-allowed" : ""
                }`}
                disabled={!canReserveFloor(location)}
              >
                <div className="aspect-video rounded-lg mb-3 overflow-hidden bg-gray-100">
                  <img
                    src={getFloorImage(location)}
                    alt={location}
                    className="w-full h-full object-cover"
                  />
                </div>
                <h3 className="font-medium text-gray-800 text-sm mb-1">{location}</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {groupedRooms[formData.location]?.map((room) => (
                ...
                >
                  ...
                  <h3 className="font-medium text-gray-800 text-sm mb-2">{room.room}</h3>
                  
                  <div className="flex flex-wrap gap-1 mb-2">
                    {room.features?.wifi && <RoomFeatureIcon feature="wifi" enabled={room.features.wifi} />}
                    {room.features?.aircon && <RoomFeatureIcon feature="aircon" enabled={room.features.aircon} />}
                    {room.features?.projector && <RoomFeatureIcon feature="projector" enabled={room.features.projector} />}
                    {room.features?.monitor && <RoomFeatureIcon feature="monitor" enabled={room.features.monitor} />}
                  </div>
        ...
        {selectedRoomDetails && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
            <div className="flex items-start">
              ...
              <div>
                <h3 className="text-blue-800 font-medium text-sm mb-1">Selected Room: {selectedRoomDetails.room}</h3>
                <p className="text-blue-700 text-xs">
                  Capacity: {selectedRoomDetails.capacity} people • Floor: {selectedRoomDetails.floor}
                </p>
                {selectedRoomDetails.features && (
                  <p className="text-blue-700 text-xs mt-1">
                    Features: {Object.entries(selectedRoomDetails.features)
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 border border-gray-100">
          <div className="flex justify-between items-center mb-3 sm:mb-4 pb-2 border-b border-gray-100">
            ...
            <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">
              {formData.numUsers} {parseInt(formData.numUsers) === 1 ? 'user' : 'users'}
            </span>
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-3 px-4 text-xs font-medium text-gray-500 uppercase tracking-wider">Participant</th>
                    <th className="text-left py-3 px-4 text-xs font-medium text-gray-500 uppercase tracking-wider">ID Number</th>
                    ...
                    <th className="text-left py-3 px-4 text-xs font-medium text-gray-500 uppercase tracking-wider">Year Level</th>
                  ...
                  {formData.participants.map((participant, idx) => (
                    <tr key={idx} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        ...
                        {!validation[idx]?.status && participant.id_number && (
                          <span className="text-gray-500 text-sm">Enter ID to verify</span>
                        )}
                      </td>
                    </tr>
        ...
        <div className="flex justify-end">
          <button
            ...
            disabled={loading}
            className="bg-[#CC0000] text-white px-6 sm:px-8 py-3 rounded-lg font-medium hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
          >
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (dayInfo && !dayInfo.disabled) {
                        setFormData({ ...formData, date: dayInfo.date });
                        setShowDateModal(false);
                      }
                    }}
                    disabled={!dayInfo || dayInfo.disabled}
                    className={`aspect-square rounded-lg text-sm font-medium transition-colors ${
                      dayInfo?.date === formData.date
            ...
            <div className="p-6">
              <div className="space-y-3">
                {[1, 2, 3, 4, 5, 6, 7, 8].map((num) => (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-gray-800">{num} {num === 1 ? 'user' : 'users'}</span>
                      {formData.numUsers === num.toString() && (
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#CC0000]" viewBox="0 0 20 20" fill="currentColor">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                        </svg>
                      )}
                    </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div className="p-6 text-center">
              ...
              <button
                ...
                className="w-full bg-[#CC0000] text-white py-3 rounded-lg font-medium hover:bg-red-700 transition-colors"
              >
                Return to Dashboard
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div className="p-6 text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-800 mb-2">Attention Needed</h3>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div className="p-6 text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-800 mb-2">Floor Access Restriction</h3>
              <p className="text-gray-600 mb-6 whitespace-pre-line">{floorAccessMessage}</p>
              
              <button
                onClick={() => setShowFloorAccessModal(false)}
                className="w-full bg-[#CC0000] text-white py-3 rounded-lg font-medium hover:bg-red-700 transition-colors"
              >
                OK
              </button>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div className="p-6 text-center">
              ...
              <p className="text-gray-600 mb-6">
                Your account is not yet verified. You can still fill out the reservation form, but your reservation will not be submitted until your account is verified.
              </p>
