...
// Check if at least the minimum number of participants are filled
const filledParticipants = formData.participants.filter(p => 
  p.name && p.department && p.id_number
).length;

const minParticipantsRequired = 3; // 3 additional + 1 main = 4 minimum
if (filledParticipants < minParticipantsRequired) {
  showAlert(`Please provide details for at least ${minParticipantsRequired} additional participants (minimum group size is 4 users).`);
  return false;
}

// Validate that filled participants are verified
for (let i = 0; i < formData.participants.length; i++) {
  const p = formData.participants[i];
  // Only validate participants that have been filled
  if (p.name && p.id_number) {
