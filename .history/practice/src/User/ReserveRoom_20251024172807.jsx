...
// When user selects number of users, it should represent additional participants
const handleNumUsersChange = (val) => {
  const n = parseInt(val, 10);
  const updated = [...formData.participants];
  const v = [...validation];
  
  // ✅ FIXED: The number represents TOTAL users (main reserver + additional)
  // So for n total users, we need n-1 additional participants
  const additionalParticipantsNeeded = n - 1;
  
  // Fill with empty participants for additional participants only
  while (updated.length < additionalParticipantsNeeded) {
    updated.push({ name: "", course: "", year_level: "", department: "", id_number: "", role: "" });
    v.push({ status: null, message: "", loading: false });
  }

  // Trim to the correct number of additional participants
  updated.length = additionalParticipantsNeeded;
  v.length = additionalParticipantsNeeded;
