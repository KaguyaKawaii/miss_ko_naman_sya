...
// When user selects number of users, it should represent ALL participants including main reserver
const handleNumUsersChange = (val) => {
  const n = parseInt(val, 10);
  const updated = [...formData.participants];
  const v = [...validation];
  
  // ✅ FIXED: The number represents ALL users including main reserver
  // So for n total users, we need n participants in the array
  const totalParticipantsNeeded = n;
  
  // Fill with empty participants for ALL participants including main reserver
  while (updated.length < totalParticipantsNeeded) {
    updated.push({ name: "", course: "", year_level: "", department: "", id_number: "", role: "" });
    v.push({ status: null, message: "", loading: false });
  }

  // Trim to the correct number of total participants
  updated.length = totalParticipantsNeeded;
  v.length = totalParticipantsNeeded;
  ...
  if (user && updated.length > 0) {
    ...
    // Set validation for main reserver
    v[0] = user.verified 
      ? { status: "valid", message: "Verified ✓", loading: false } 
      : { status: "invalid", message: "Not Verified", loading: false };
  }
...
const validateForm = () => {
  ...
  // ✅ FIXED: Get the total expected participants based on selected group size
  const totalUsers = parseInt(formData.numUsers);
  const expectedParticipants = totalUsers - 1; // Excluding main reserver

  // Check if we have the exact number of participants for the selected group size
  if (formData.participants.length !== expectedParticipants) {
    showAlert(`Form error: Expected ${expectedParticipants} participants for ${totalUsers} users. Please refresh the page and try again.`);
    return false;
  }

  // ✅ FIXED: Count how many participants are actually filled (have name and ID)
  const filledParticipants = formData.participants.filter(p => 
    p.name && p.name.trim() && p.id_number && p.id_number.toString().trim()
  ).length;

  // ✅ FIXED: For the selected group size, ALL participants must be filled
  if (filledParticipants !== expectedParticipants) {
    showAlert(`Please complete all ${expectedParticipants} participant fields for ${totalUsers} total users.`);
    return false;
  }
  ...
  for (let i = 0; i < formData.participants.length; i++) {
    ...
    // Check if all required fields are filled (should be true based on above check, but double-check)
...
const reservationData = {
  ...
  numUsers: parseInt(formData.numUsers), // This should be total including main reserver
  purpose: formData.purpose,
  participants: formData.participants, // These are the additional participants only
                  ...
                  <div key={index} className="text-center">
                    {day ? (
                      <button
                        ...
                        className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors text-base font-medium
                          ${day.disabled ? 
                            day.isSunday ? 
