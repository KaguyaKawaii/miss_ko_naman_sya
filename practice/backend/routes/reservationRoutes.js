...
router.post("/", async (req, res) => {
  try {
    ...
    const formattedDate = new Date(reservation.date).toLocaleDateString("en-PH", { year: "numeric", month: "long", day: "numeric" });
...
router.get("/active/:userId", async (req, res) => {
  try {
    const { userId } = req.params;
    const now = new Date();
    const activeReservation = await Reservation.findOne({ userId, endDatetime: { $gt: now } });
